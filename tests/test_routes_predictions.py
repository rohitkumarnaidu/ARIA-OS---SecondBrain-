...
class TestPredictionRoutes:
    ...
    async def test_predict_task_completion_success(self, mock_supabase_cli):
        ...
        mock_client.from_.return_value.select.return_value.eq.return_value.order.return_value.execute.return_value.data = [
            {
                "id": "t1",
    ...
    @patch("app.api.predictions.get_supabase_client")
    @pytest.mark.asyncio
    async def test_predict_task_completion_due_today_and_overdue(self, mock_supabase_cli):
        from datetime import datetime, timezone, timedelta

        mock_client = MagicMock()
        mock_supabase_cli.return_value = mock_client
        today_iso = (datetime.now(timezone.utc) + timedelta(hours=6)).isoformat().replace("+00:00", "Z")
        mock_client.from_.return_value.select.return_value.eq.return_value.order.return_value.execute.return_value.data = [
            {
                "id": "t1",
                "title": "Due today",
    ...
    async def test_predict_task_completion_db_error(self, mock_supabase_cli):
        mock_client = MagicMock()
        mock_supabase_cli.return_value = mock_client
        mock_client.from_.return_value.select.return_value.eq.return_value.order.return_value.execute.side_effect = (
            Exception("DB error")
        )
        from app.api.predictions import predict_task_completion
    ...
    async def test_predict_sleep_success(self, mock_supabase_cli):
        ...
        mock_client.from_.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value.data = (
            logs
        )
    ...
    async def test_predict_sleep_db_error(self, mock_supabase_cli):
        mock_client = MagicMock()
        mock_supabase_cli.return_value = mock_client
        mock_client.from_.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.side_effect = Exception(
            "DB error"
        )
        from app.api.predictions import predict_sleep
        from fastapi import HTTPException
    ...
    async def test_predict_smart_slots(self, mock_supabase_cli):
        ...
        mock_supabase_cli.return_value = mock_client
        mock_client.from_.return_value.select.return_value.eq.return_value.execute.return_value.data = [
            {"start_time": "2026-06-22T09:00:00Z", "duration_minutes": 120, "id": "e1"},
            {"start_time": "2026-06-22T10:00:00Z", "duration_minutes": 60, "id": "e2"},
            {"start_time": "2026-06-23T14:00:00Z", "duration_minutes": 90, "id": "e3"},
