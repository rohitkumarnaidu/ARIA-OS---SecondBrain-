...
class TestFeatureFlagRoutes:
    ...
    @pytest.mark.asyncio
    async def test_list_feature_flags_success(self, mock_flags, mock_flag):
        mock_flags.all_flags.return_value = {"test-flag": mock_flag}
        from app.api.feature_flags import list_feature_flags

        result = await list_feature_flags(user_id="test-user")
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_list_feature_flags_empty(self, mock_flags):
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_get_feature_flag_success(self, mock_flags, mock_flag):
        mock_flags._flags = {"test-flag": mock_flag}
        from app.api.feature_flags import get_feature_flag

        result = await get_feature_flag(key="test-flag", user_id="test-user")
        assert result["key"] == "test-flag"

    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_get_feature_flag_not_found(self, mock_flags):
        mock_flags._flags = {}
        from app.api.feature_flags import get_feature_flag
        from fastapi import HTTPException

        with pytest.raises(HTTPException) as exc:
            await get_feature_flag(key="nonexistent", user_id="test-user")
        assert exc.value.status_code == 404

    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_evaluate_feature_flag(self, mock_flags):
        ...
        result = await evaluate_feature_flag(key="test-flag", user_id="test-user")
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_create_feature_flag_success(self, mock_flags, mock_ff_class, mock_flag):
        mock_flags._flags = {}
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_create_feature_flag_conflict(self, mock_flags, mock_ff_class, mock_flag):
        mock_flags._flags = {"existing": mock_flag}
        from app.api.feature_flags import create_feature_flag
        from database.schemas.feature_flag import FeatureFlagCreate
        from fastapi import HTTPException
        ...
        with pytest.raises(HTTPException) as exc:
            await create_feature_flag(flag_data=flag_data, user_id="test-user")
        assert exc.value.status_code == 409

    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_update_feature_flag_success(self, mock_flags, mock_flag):
        mock_flags._flags = {"test-flag": mock_flag}
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_update_feature_flag_all_fields(self, mock_flags, mock_flag):
        mock_flags._flags = {"test-flag": mock_flag}
        from app.api.feature_flags import update_feature_flag
        from database.schemas.feature_flag import FeatureFlagUpdate
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_update_feature_flag_not_found(self, mock_flags):
        mock_flags._flags = {}
        from app.api.feature_flags import update_feature_flag
        from database.schemas.feature_flag import FeatureFlagUpdate
        from fastapi import HTTPException
    ...
    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_delete_feature_flag_success(self, mock_flags):
        mock_flags.delete.return_value = True
        from app.api.feature_flags import delete_feature_flag

        result = await delete_feature_flag(key="test-flag", user_id="test-user")
        assert result is None
        mock_flags.delete.assert_called_once_with("test-flag")

    @patch("app.api.feature_flags.flags")
    @pytest.mark.asyncio
    async def test_delete_feature_flag_not_found(self, mock_flags):
        mock_flags.delete.return_value = False
        from app.api.feature_flags import delete_feature_flag
        from fastapi import HTTPException

        with pytest.raises(HTTPException) as exc:
            await delete_feature_flag(key="nonexistent", user_id="test-user")
        assert exc.value.status_code == 404
