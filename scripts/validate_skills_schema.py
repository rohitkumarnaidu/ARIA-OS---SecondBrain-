...
REQUIRED_MVS = [
    "mv_skill_user_proficiency",
    "mv_skill_market_intelligence",
    "mv_skill_learning_velocity",
    "mv_skill_taxonomy_health",
    "mv_skill_ai_effectiveness",
    "mv_skill_activity_heatmap",
...
class SchemaValidator:
    ...
    def validate_generator_script(self, gen_path: Path):
        ...
        if exists:
            ...
            self.check("def main()" in content, f"Generator script missing main() function: {gen_path}")
    ...
    def validate_ddl_coverage(self, ddl_path: Path):
        ...
        for table in REQUIRED_TABLES:
            ...
            self.check(pattern in content, f"Missing CREATE TABLE for {table} in DDL")

        for role in REQUIRED_ROLES:
            pattern = f"CREATE ROLE {role}"
            self.check(pattern in content, f"Missing CREATE ROLE {role} in DDL")
        ...
        for table in RLS_ENABLED_TABLES:
            ...
            self.check(has_rls_loop or pattern in content, f"Missing RLS enable for {table} in DDL")
        ...
        for table in PARTITIONED_TABLES:
            ...
            if pattern in content:
                ...
                blocks = re.findall(rf"CREATE TABLE IF NOT EXISTS {table}.*?\);", content, re.DOTALL)
                for block in blocks:
                    if "PARTITION BY RANGE" in block:
                        present = True
                        break
            self.check(present, f"Table {table} should be partitioned but PARTITION BY RANGE not found in its DDL")
        ...
        for table, generated_cols in GENERATED_COLUMN_TABLES.items():
            for col in generated_cols:
                ...
                col_pattern = re.search(rf"{re.escape(col)}\s+\w+\s+GENERATED\s+ALWAYS\s+AS", content, re.IGNORECASE)
                self.check(bool(col_pattern), f"Missing GENERATED column {col} on {table}")

        for table, constraint in EXCLUDE_CONSTRAINT_TABLES.items():
            self.check(constraint in content, f"Missing EXCLUDE constraint {constraint} on {table}")
    ...
    def validate_notify_triggers(self, ddl_path: Path):
        ...
        for func_name, desc in expected_notify:
            self.check(func_name in content, f"Missing {desc} in DDL")
    ...
    def validate_audit_triggers(self, ddl_path: Path):
        ...
        for table in AUDIT_TRIGGER_TABLES:
            self.check(f"trg_{table}_audit" in content, f"Missing audit trigger for {table}")
    ...
    def validate_partitions(self, ddl_path: Path):
        ...
        for table in PARTITIONED_TABLES:
            ...
            self.check(pattern in content, f"Missing partition children for {table}")
    ...
    def validate_cron_jobs(self, cron_path: Path):
        ...
        for job in expected_jobs:
            self.check(job in content, f"Missing pg_cron job: {job}")

    def report(self) -> int:
        print(f"\n{'='*60}")
        print("SKILLS DATABASE SCHEMA VALIDATION REPORT")
        print(f"{'='*60}")
...
def main():
    ...
    parser.add_argument(
        "--migrations-dir", default="scripts/migrations", help="Directory containing migration SQL files"
    )
    parser.add_argument("--gen-script", default="scripts/gen_sdb_full.py", help="Path to generator script")
    parser.add_argument(
        "--ddl", default=None, help="DDL file to validate (defaults to migrations-dir/000_skills_complete_ddl.sql)"
    )
    parser.add_argument(
        "--db", action="store_true", help="Also validate against live database (requires --dsn or env DATABASE_URL)"
    )
    ...
    if schema_path.exists():
        ...
        if missing_models:
            validator.check(len(missing_models) == 0, f"Missing Pydantic models: {', '.join(missing_models)}", "warn")
