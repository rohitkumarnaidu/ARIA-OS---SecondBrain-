...
CHECKLIST = {
    "000_skills_complete_ddl.sql": [
        ...
        ("mv_skill_ai_effectiveness (or full name)",
         r"mv_skill_ai_effectiveness\b"),
    ...
    "001_skills_core_taxonomy.sql": [
        ...
        ("EXCLUDE on skill_relationships", r"EXCLUDE USING gist"),
        ("7 tables created",
         lambda t: len(re.findall(r"CREATE TABLE IF NOT EXISTS", t, re.I)) >= 7),
    ],
    "002_skills_user_tables.sql": [
        ("btree_gist extension", r"CREATE EXTENSION IF NOT EXISTS.*btree_gist"),
        ("5 tables created",
         lambda t: len(re.findall(r"CREATE TABLE IF NOT EXISTS", t, re.I)) >= 5),
        ...
        ("evidence partition child tables",
         r"CREATE TABLE IF NOT EXISTS user_skill_evidence_2026"),
        ("versions partition child tables",
         r"CREATE TABLE IF NOT EXISTS user_skill_versions_2026"),
    ],
    "003_skills_intelligence_supporting.sql": [
        ("btree_gist extension", r"CREATE EXTENSION IF NOT EXISTS.*btree_gist"),
        ("11 tables created",
         lambda t: len(re.findall(r"CREATE TABLE IF NOT EXISTS", t, re.I)) >= 11),
        ("EXCLUDE on certifications",
         lambda t: bool(re.search(r"EXCLUDE USING gist", t, re.I)),
         "EXCLUDE USING gist on skill_certifications"),
        ("activity_log partition child tables",
         r"CREATE TABLE IF NOT EXISTS skill_user_activity_log_2026"),
    ],
    "004_skills_audit_events_analytics.sql": [
        ("10 tables created",
         lambda t: len(re.findall(r"CREATE TABLE IF NOT EXISTS", t, re.I)) >= 10),
    ...
    "005_skills_security_rls.sql": [
        ("8 roles created",
         lambda t: len(re.findall(r"CREATE ROLE skill_", t, re.I)) >= 8),
        ("RLS enabled on 31+ tables",
         lambda t: len(re.findall(r"ENABLE ROW LEVEL SECURITY", t, re.I)) >= 31),
        ("FORCE RLS on 31+ tables",
         lambda t: len(re.findall(r"FORCE ROW LEVEL SECURITY", t, re.I)) >= 31),
        ...
        ("8 audit triggers",
         lambda t: len(re.findall(r"CREATE TRIGGER.*_audit", t, re.I)) >= 8),
        ("PII encryption functions", r"skill_encrypt_pii"),
        ("PII decryption functions", r"skill_decrypt_pii"),
    ],
    "006_skills_materialized_views.sql": [
        ("7 materialized views",
         lambda t: len(re.findall(r"CREATE MATERIALIZED VIEW", t, re.I)) >= 7),
...
def check_cross_reference(files: dict[str, str]) -> tuple[int, int, list[str]]:
    ...
    for fname, sql in files.items():
        ...
        for m in re.finditer(
            ...
            check(has_pattern(sql_000, ref_pat),
                  f"Table '{tbl}' from {fname} present in 000")
