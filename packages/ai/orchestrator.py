...
Break this query down into a step-by-step plan."""
        ...
        steps = [
            {"action": "search", "target": "all", "reasoning": "Fallback: search across all modules", "confidence": 0.5}
        ]
...
async def detect_patterns(user_id: str, query: str) -> Dict[str, Any]:
    try:
        ...
        habits = (
            ...
            .select("id, name, is_active, current_streak, best_streak, consistency_percentage")
            .eq("user_id", user_id)
            .execute()
            .data
            or []
        ...
        sleep_logs = (
            ...
            .select("date, sleep_score, duration_hours")
            .eq("user_id", user_id)
        ...
        time_entries = (
            ...
            .select("duration_minutes, is_deep_work, category, start_time")
            .eq("user_id", user_id)
    ...
    patterns = [
        {
            ...
            "description": f"Task completion rate: {completion_rate}%",
            "confidence": 0.8,
            "data": {"rate": completion_rate, "total": total_tasks, "completed": completed},
        },
        {
            "type": "task_category_breakdown",
            "description": f"Tasks by category: {dict((k, v['total']) for k, v in by_category.items())}",
            "confidence": 0.7,
            "data": by_category,
        },
        {
            "type": "habit_consistency",
            "description": f"Habit consistency: {avg_consistency}% across {len(active_habits)} active habits",
            "confidence": 0.75,
            "data": {"consistency": avg_consistency, "active_count": len(active_habits)},
        },
        {
            "type": "sleep_quality",
            "description": f"Avg sleep score: {avg_sleep_score}, avg duration: {avg_duration}h",
            "confidence": 0.6,
            "data": {"avg_score": avg_sleep_score, "avg_duration": avg_duration},
        },
        {
            "type": "focus_time",
            "description": f"Deep work: {deep_work_minutes}m of {total_minutes}m tracked",
            "confidence": 0.7,
            "data": {"deep_work_minutes": deep_work_minutes, "total_minutes": total_minutes},
        },
    ]
        ...
        insight_items.append(
            {
                ...
                "description": "Task completion rate is below 50% — consider reducing workload or breaking tasks down",
                "confidence": 0.6,
        ...
        insight_items.append(
            {
                ...
                "description": "Habit consistency is low — focus on building smaller daily habits",
                "confidence": 0.6,
                "data": {"consistency": avg_consistency},
            }
        )
        ...
        insight_items.append(
            {
                ...
                "description": "Sleep quality is below optimal — consider a consistent bedtime routine",
                "confidence": 0.5,
                "data": {"avg_score": avg_sleep_score},
            }
        )
...
async def match_opportunities(user_id: str, query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        roadmap = (
            supabase.from_("roadmap").select("skill_name, status").eq("user_id", user_id).limit(20).execute().data or []
        )
        for r in roadmap:
        ...
        matches = [
            {
                "id": o["id"],
                "title": o["title"],
                "score": o.get("match_score", 0.5),
                "reasoning": "Fallback scoring based on stored match_score",
            }
        ...
        if action_type == "create_task":
            ...
            if not task_id:
                return {
                    ...
                    "result": {"error": "No matching task found"},
            ...
            if not habit_id:
                return {
                    ...
                    "result": {"error": "No matching habit found"},
