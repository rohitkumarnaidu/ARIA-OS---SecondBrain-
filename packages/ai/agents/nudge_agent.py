...
NUDGE_TYPES = ["course_behind", "habit_miss_2day", "habit_miss_5day",
               "multiple_courses_behind", "streak_at_risk"]
...
async def generate_nudge(user_id: str, nudge_type: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if nudge_prompt:
        ...
        user = (
            f"Nudge type: {nudge_type}. Context: {context}. "
            f"Return JSON with nudge_text and smallest_action."
        )
...
async def check_course_nudges(user_id: str) -> List[Dict[str, Any]]:
    ...
    for course in courses:
        if course.get("progress", 0) < 30 and course.get("status") == "in_progress":
            nudge = await generate_nudge(user_id, "course_behind", {
...
async def check_habit_streaks(user_id: str) -> List[Dict[str, Any]]:
    ...
    for habit in habits:
        ...
        if streak >= 7:
            nudge = await generate_nudge(user_id, "streak_at_risk", {
                "name": habit.get("name"),
            ...
            nudge = await generate_nudge(user_id, "habit_miss_5day", {
                "name": habit.get("name"),
                "missed_days": habit.get("missed_days", 0),
            })
            nudges.append(nudge)
        elif habit.get("missed_days", 0) >= 2:
            nudge = await generate_nudge(user_id, "habit_miss_2day", {
                "name": habit.get("name"),
                "missed_days": habit.get("missed_days", 0),
            })
            nudges.append(nudge)
