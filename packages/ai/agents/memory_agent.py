...
async def store_interaction(user_id: str, interaction_type: str, content: str, metadata: Dict[str, Any] = None) -> dict:
    ...
    data = {"user_id": user_id, "interaction_type": interaction_type, "content": content,
...
async def get_recent_interactions(user_id: str, limit: int = 10) -> List[dict]:
    supabase = get_supabase_client()
    response = (supabase.from_("memory").select("*").eq("user_id", user_id)
                .order("timestamp", ascending=False).limit(limit).execute())
...
async def update_memory_context(user_id: str, context: Dict[str, Any]) -> None:
    ...
    if existing.data:
        supabase.from_("user_context").update({"context": context, "updated_at": datetime.now().isoformat()}).eq("user_id", user_id).execute()
    else:
