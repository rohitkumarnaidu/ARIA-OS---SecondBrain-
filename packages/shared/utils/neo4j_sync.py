...
class Neo4jSyncService:
    ...
    async def full_rebuild(self, skills: list[dict], categories: list[dict],
                           relationships: list[dict], user_skills: list[dict],
                           evidence: list[dict], targets: list[dict],
    ...
    async def get_graph_statistics(self) -> dict:
        ...
        return {
            ...
            "node_distribution": {
                ",".join(lbl["labels"]): lbl["count"] for lbl in labels
            } if labels else {},
        }
