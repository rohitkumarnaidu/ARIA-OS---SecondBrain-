    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-text-secondary">Build and launch your ideas</p>
        </div>
      ...
      >
        {phases.map(p => (
          <motion.div key={p} variants={itemVariants} className="card p-3 text-center">
            <div className="text-xl font-bold text-text-primary">{phaseCounts[p]}</div>
            <div className="text-xs text-text-muted capitalize">{p}</div>
          </motion.div>
        ))}
      </motion.div>
      ...
      {projects.length === 0 && (
        ...
        >
          <FolderKanban size={48} className="text-text-muted mx-auto mb-3" />
