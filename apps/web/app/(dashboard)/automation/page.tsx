...
import { Play, Zap, Target, Clock, Moon, Bell, RefreshCw, CheckCircle, XCircle, Settings } from 'lucide-react'
...
export default function AutomationPage() {
  ...
  const runAutomation = async (type: string) => {
    ...
    try {
      const endpoint = type === 'briefing'
        ...
        : type === 'sleep_analysis'
        ? '/api/automation/trigger/sleep-analysis'
        : type === 'sleep_bedtime'
        ? '/api/automation/trigger/sleep-bedtime'
        : '/api/automation/trigger/nudges'
  ...
  const automations = [
    ...
    { id: 'sleep_analysis', name: 'Sleep Analysis', description: 'Analyze last night sleep quality and get personalized recommendations', icon: Moon, schedule: 'On-demand' },
    { id: 'sleep_bedtime', name: 'Bedtime Suggestion', description: 'Get AI-suggested bedtime based on your recent sleep patterns', icon: Moon, schedule: 'On-demand' },
    { id: 'nudges', name: 'Course & Habit Nudges', description: 'Check course progress and habit streaks, generate motivational nudges', icon: Bell, schedule: '6 PM daily' },
  ]
      ...
      <motion.div variants={itemVariants}>
        <div className="card">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-background-elevated rounded-lg">
              <div className="flex items-center gap-3">
                <Moon className="text-accent-primary" size={18} />
                <span className="text-text-primary">Sleep Reminder</span>
              </div>
              <span className="text-text-muted text-sm">10:30 PM daily</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-background-elevated rounded-lg">
              <div className="flex items-center gap-3">
                <Bell className="text-accent-primary" size={18} />
                <span className="text-text-primary">Course Progress Nudge</span>
              </div>
              <span className="text-text-muted text-sm">6:00 PM daily</span>
            </div>
          </div>
        </div>
      </motion.div>
