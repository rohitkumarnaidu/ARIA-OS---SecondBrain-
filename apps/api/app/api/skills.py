...
from database.schemas.skill import (
    ...
    TagResponse,
    SkillTagCreate,
    SkillExternalMappingCreate,
    SkillExternalMappingUpdate,
    SkillExternalMappingResponse,
    SkillRoadmapDefinitionCreate,
    SkillRoadmapDefinitionResponse,
    UserSkillCreate,
    UserSkillUpdate,
    UserSkillResponse,
    UserSkillEvidenceCreate,
    UserSkillEvidenceResponse,
    UserSkillTargetCreate,
    UserSkillTargetResponse,
    UserSkillAssessmentCreate,
    UserSkillAssessmentResponse,
    SkillMarketDataCreate,
    SkillMarketDataResponse,
    SkillIncomeDataCreate,
    SkillIncomeDataResponse,
    SkillCertificationCreate,
    SkillCertificationResponse,
    SkillProjectLinkCreate,
    SkillRoadmapLinkCreate,
    SkillOpportunityLinkCreate,
    SkillTopicCreate,
    SkillTopicResponse,
    SkillResourceCreate,
    SkillResourceResponse,
    SkillLearningPathCreate,
    SkillLearningPathResponse,
    SkillAIRecommendationResponse,
    SkillActivityLogCreate,
    SkillActivityLogResponse,
    SkillEventCreate,
    SkillEventResponse,
    SkillForecastCreate,
    SkillForecastResponse,
    SkillAuditLogResponse,
    SkillEventSubscriptionCreate,
    SkillEventSubscriptionUpdate,
    SkillEventSubscriptionResponse,
    SkillAnalyticsSnapshotResponse,
    SkillWebhookQueueResponse,
    SkillTaxonomyHistoryResponse,
    SkillUserSkillHistoryResponse,
    SkillMarketHistoryResponse,
)

router = APIRouter()


def _get_supabase():  # pragma: no cover
...
async def list_categories(
    ...
    response = (
        supabase.from_("skill_categories").select("*").range(offset, offset + limit - 1).order("sort_order").execute()
    )
...
async def list_tags(
    current_user=Depends(get_current_user), limit: int = Query(100, ge=1, le=500), offset: int = Query(0, ge=0)
):
    supabase = _get_supabase()
...
async def list_assessments(
    ...
    user_id = current_user.user.id
    query = (
        supabase.from_("user_skill_assessments").select("*").eq("user_id", user_id).range(offset, offset + limit - 1)
    )
    if user_skill_id:
        query = query.eq("user_skill_id", user_skill_id)
    response = query.execute()
...
async def list_recommendations(
    ...
    user_id = current_user.user.id
    query = (
        supabase.from_("skill_ai_recommendations").select("*").eq("user_id", user_id).range(offset, offset + limit - 1)
    )
...
async def accept_recommendation(recommendation_id: str, current_user=Depends(get_current_user)):
    supabase = _get_supabase()
    response = (
        supabase.from_("skill_ai_recommendations")
...
async def list_activity(
    ...
    user_id = current_user.user.id
    query = (
        supabase.from_("skill_user_activity_log")
        .select("*")
        .eq("user_id", user_id)
...
async def list_roadmap_definitions(
    current_user=Depends(get_current_user), limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0)
):
    supabase = _get_supabase()
...
async def update_roadmap_definition(
    roadmap_id: str, update: SkillRoadmapDefinitionCreate, current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
...
async def update_certification(
    certification_id: str, update: SkillCertificationCreate, current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
    payload = update.model_dump()
...
async def delete_project_skill_link(
    project_id: str = Query(...), skill_id: str = Query(...), current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
...
async def delete_roadmap_skill_link(
    roadmap_id: str = Query(...), skill_id: str = Query(...), current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
...
async def delete_opportunity_skill_link(
    opportunity_id: str = Query(...), skill_id: str = Query(...), current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
    response = (
        supabase.from_("skill_opportunities")
        .delete()
...
async def update_external_mapping(
    mapping_id: str, update: SkillExternalMappingUpdate, current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
    payload = update.model_dump()
...
async def list_audit_log(
    ...
    supabase = _get_supabase()
    query = (
        supabase.from_("skill_audit_log").select("*").range(offset, offset + limit - 1).order("created_at", desc=True)
    )
    if table_name:
...
async def get_audit_for_record(table_name: str, record_id: str, current_user=Depends(get_current_user)):
    supabase = _get_supabase()
    response = (
        supabase.from_("skill_audit_log")
        .select("*")
...
async def update_event_subscription(
    subscription_id: str, update: SkillEventSubscriptionUpdate, current_user=Depends(get_current_user)
):
    supabase = _get_supabase()
    update_data = {k: v for k, v in update.model_dump().items() if v is not None}
    response = (
        supabase.from_("skill_event_subscriptions").update(update_data).eq("subscription_id", subscription_id).execute()
    )
    if not response.data:
        raise HTTPException(status_code=404, detail="Subscription not found")
    return response.data[0]
...
async def list_webhook_queue(
    ...
    supabase = _get_supabase()
    query = (
        supabase.from_("skill_webhook_queue")
        .select("*")
        .range(offset, offset + limit - 1)
        .order("created_at", desc=True)
    )
...
async def list_taxonomy_history(
    ...
    supabase = _get_supabase()
    query = (
        supabase.from_("skill_taxonomy_history")
        .select("*")
        .range(offset, offset + limit - 1)
        .order("created_at", desc=True)
    )
...
async def list_user_skill_history(
    ...
    supabase = _get_supabase()
    query = (
        supabase.from_("skill_user_skill_history")
        .select("*")
        .range(offset, offset + limit - 1)
        .order("created_at", desc=True)
    )
...
async def list_market_history(
    ...
    supabase = _get_supabase()
    query = (
        supabase.from_("skill_market_history")
        .select("*")
        .range(offset, offset + limit - 1)
        .order("created_at", desc=True)
    )
...
async def get_market_intelligence(current_user=Depends(get_current_user)):
    supabase = _get_supabase()
    response = (
        supabase.from_("mv_skill_market_intelligence").select("*").order("skill_health", desc=True).limit(50).execute()
    )
    return response.data
