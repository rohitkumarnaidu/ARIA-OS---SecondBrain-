...
**What exists today:**
- `next-pwa` (`^5.6.0`) is installed in `package.json` but **not configured** in `next.config.js`
...
2. Service worker design with `next-pwa` + Workbox
...
```javascript
// public/sw.js — auto-generated by next-pwa, with custom additions

// Workbox runtime is injected by next-pwa
...
| Configure `next-pwa` in `next.config.js` | 1h | None |
...
| Implement `useSWUpdate` + update banner | 2h | next-pwa |
