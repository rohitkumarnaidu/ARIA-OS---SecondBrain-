...
**Document ID:** GOV-CMG-002
**Version:** 2.0.0
**Effective Date:** 2026-07-11
**Supersedes:** OPS-CM-001 (v1.0.0)
...
This document defines the change management and release process for Second Brain OS â€” a personal AI productivity system built on Next.js, FastAPI, Supabase, and AI services. The purpose is to manage all changes to the system predictably, minimize production incidents, maintain code quality, and establish a lightweight yet repeatable release cadence suitable for a single-developer team.

### 1.2 Scope

This process governs the following categories of change:

- **Code changes** â€” Frontend (Next.js), Backend (FastAPI), shared packages, AI agent modules, and service-layer code.
- **Infrastructure changes** â€” Hosting platform migrations (Vercel, Railway), Supabase project configuration, DNS, CDN, and deployment pipeline modifications.
- **Configuration changes** â€” Environment variables, feature flags, application settings, and third-party service keys.
- **AI prompt changes** â€” Agent instructions, system prompts, briefing templates, and model parameter adjustments.
- **Dependency updates** â€” npm packages, Python packages, Supabase client libraries, and runtime version upgrades.
- **Governance changes** â€” Updates to standards, ownership, review schedules, or DR procedures.

### 1.3 Change Management Philosophy

The process is designed to be **lightweight but structured** â€” sufficient rigor to prevent regressions and data loss without imposing bureaucratic overhead on a solo developer. The guiding principles are:

1. **Risk-appropriate process** â€” Higher-risk changes require more gates (branching, testing, dry-run). Low-risk changes remain fast and frictionless.
2. **Traceability by default** â€” Every change is linked to an issue, labeled by type and risk, and recorded in git history.
3. **Recovery over perfection** â€” Rollback procedures and emergency processes are pre-defined so the developer can act quickly when things go wrong.
4. **Continuous improvement** â€” Metrics are tracked and retrospectives are held to refine the process over time.
...
| **Governance** | Changes to process, standards, or controls | Updating DR plan, changing review cadence |
...
|---|---|---|---|
| **P0 â€” Critical** | System is down, data is at risk, or authentication is broken | Auth middleware failing, database corruption, secrets leaked | Immediate |
| **P1 â€” High** | Major feature is broken with no workaround | Task creation returning 500, AI chat non-responsive | < 4 hours |
| **P2 â€” Medium** | Feature is partially broken; workaround exists | Dashboard stats incorrect, search missing some results | < 24 hours |
| **P3 â€” Low** | Cosmetic issue or minor usability problem | Button alignment off, typo in UI text | Next release |
| **P4 â€” Enhancement** | New feature or improvement request | Dark mode toggle, export to CSV feature | Backlog |
...
```
Idea/Bug â†’ GitHub Issue â†’ Plan â†’ Branch â†’ Code â†’ Test â†’ PR â†’ Review â†’ Merge â†’ Deploy â†’ Verify â†’ Close Issue
```
...
## Motivation
[Why this change is needed â€” bug, feature request, tech debt, etc.]

## Implementation Plan
[Brief outline of approach â€” required for Medium+ changes]
...
**Labeling Convention:**
- `type: feature`, `type: fix`, `type: refactor`, `type: dependency`, `type: config`, `type: infrastructure`, `type: docs`, `type: ai`, `type: security`, `type: governance`
...
| Medium | Brief implementation plan as issue comment (3â€“5 bullet points) |
...
1. **Skip issue creation** â€” an issue can be created after resolution for tracking.
...
3. **Fix, test minimally**, and merge with a `HOTFIX:` prefix in the merge commit:
   ...
   ```
4. **Deploy immediately** â€” bypass staging and dry-run.
5. **Create issue after** for post-incident tracking.
6. **Document** in the incident response log (see `docs/operations/40_IncidentResponse.md`).
...
|---|---|---|---|
| **Patch Release** | As needed (typically 1â€“3 per week) | P0/P1 fixes, low-risk changes, documentation | PATCH |
...
```
MAJOR â€” Breaking API changes, database schema migrations that are backward-incompatible,
        major redesigns, or removal of features
MINOR â€” New features, non-breaking enhancements, new API endpoints, new pages
PATCH â€” Bug fixes, performance improvements, documentation, refactors, dependency updates

Current Version: 0.1.0 (pre-alpha)
```

**Version Source of Truth:**
- Frontend: `apps/web/package.json` â€” `"version": "0.1.0"`
- Backend: `apps/api/pyproject.toml` â€” `version = "0.1.0"`
...
- [ ] Version displayed in footer matches
...
| **npm packages** | zod, zustand, framer-motion | Monthly via `npm audit` | Security patches: immediately; others: within 1 month |
...
| **AI/LLM** | Claude API, Ollama, model IDs | Per deprecation notice | Update within 30 days of deprecation notice |
...
- `SUPABASE_SERVICE_ROLE_KEY` â€” Supabase service role key (backend only)
- `CLAUDE_API_KEY` â€” Anthropic API key for AI features
- `OPENROUTER_API_KEY` â€” OpenRouter API key (if used)
- `SESSION_SECRET` â€” Session encryption key
- `NEXT_PUBLIC_APP_URL` â€” Public application URL
...
3. **Production update:**
   - Vercel: Dashboard â†’ Project â†’ Settings â†’ Environment Variables â†’ Update
   - Railway: Dashboard â†’ Project â†’ Variables â†’ Update
...
- **Never** commit `.env` files to the repository â€” they are listed in `.gitignore`.
...
|---|---|---|---|
| **Build fails on production** | Vercel deploy log, CI failure | Auto-rollback to previous deployment | Developer |
| **API error rate > 5%** | Railway monitoring, log analysis | Manual rollback to previous Railway deployment | Developer |
| **Authentication broken** | User report, monitoring alert | Emergency rollback â€” deploy previous commit | Developer |
| **Data corruption detected** | Manual audit, test failure | Restore from database backup (see DR plan) | Developer |
| **Performance degradation > 50%** | Lighthouse CI, response time monitoring | Rollback to previous deployment | Developer |
...
# Alternative: Vercel Dashboard â†’ Deployments â†’ ... â†’ Rollback to Previous

# Backend â€” Railway
railway redeploy --deployment <id>       # Deploy specific previous deployment
# Alternative: Railway Dashboard â†’ Deployments â†’ Select previous â†’ Redeploy
...
- **CHANGELOG.md** â€” Human-readable release notes.
- **Git tags** â€” Annotated tags with release notes (`git tag -a v0.2.0 -m "..."`).
- **GitHub Issues** â€” Complete traceability from request to deployment.
- **Deployment logs** â€” Vercel and Railway deployment history.
...
|---|---|---|---|
| **Lead Time** | Time from issue creation to deployment | < 2 days | GitHub Issue created â†’ merge commit date |
| **Deployment Frequency** | Number of production deployments per week | 2â€“4 per week | Git log, Vercel deployments |
| **Change Failure Rate** | Percentage of deployments causing a failure | < 5% | Deployments followed by rollback or hotfix |
| **Time to Restore (P0)** | Time from P0 incident to resolution | < 4 hours | Incident created â†’ fix deployed |
...
```markdown
## Retrospective â€” [Release/Incident Name]
...
### Action Items
- [ ] [Action item 1] â€” Owner, Due date
- [ ] [Action item 2] â€” Owner, Due date
...
### Change Summary
**Title:** {type}: {short description}
**Type:** {Feature / Fix / Refactor / Dependency / Config / Infrastructure / Docs / AI / Security / Governance}
...
- **Database Changes:** {yes/no â€” details if yes}
- **Breaking Changes:** {yes/no â€” details if yes}
...
```
# SECOND BRAIN OS â€” RELEASE CHECKLIST
...
| `governance` | Process or standards change | `governance: update DR plan` |
...
Is this change BREAKING?
  â”œâ”€â”€ Yes â†’ MAJOR bump
  â””â”€â”€ No â†’ Is it a NEW FEATURE?
       â”œâ”€â”€ Yes â†’ MINOR bump
       â””â”€â”€ No â†’ Is it a BUG FIX, REFACTOR, or DOCS?
            â”œâ”€â”€ Yes â†’ PATCH bump
            â””â”€â”€ No â†’ Evaluate manually
```
...
|---|---|---|---|
| 1.0.0 | 2026-06-11 | Platform Engineering | Initial release |
| 2.0.0 | 2026-07-11 | Developer | Moved from `docs/operations/` to `docs/governance/`. Updated Document ID to GOV-CM-001. Added governance change type. Updated rollback references to DR plan. Added `governance` commit type. |
