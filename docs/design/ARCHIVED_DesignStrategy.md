# Design Strategy â€” Second Brain OS (ARIA OS)
...
| Field | Value |
|---|---|
| Document ID | DSG-ADS-001 |
| Version | 1.0.0 |
| Status | Active |
| Last Updated | 2026-06-11 |
| Classification | Internal â€” Design Leadership |
...
graph TD
    subgraph Pillar_1["ðŸŽ¯ Product & Vision"]
        A["Executive Design Vision"]
        B["Product Personality"]
        C["Brand Direction"]
    end
    subgraph Pillar_2["ðŸŽ¨ Visual & Interaction"]
        D["Visual Strategy"]
        E["Interaction Strategy"]
        F["AI Design Strategy"]
    end
    subgraph Pillar_3["ðŸ“± Platform & Experience"]
        G["Dashboard Strategy"]
        H["Navigation Strategy"]
        I["Mobile / Tablet / Desktop"]
    end
    subgraph Pillar_4["âš™ï¸ Architecture & Scale"]
...
3. **AI is a material, not a feature.** Like glass, light, or motion â€” AI is something we design with, not something we add on.
...
```
DesignStrategy.md (THIS FILE â€” Strategic Constitution)
  â”œâ”€â”€ Design.md (Design Architecture â€” patterns, specs, components)
  â”‚   â”œâ”€â”€ Antigravity (Design System â€” tokens, atoms, molecules, organisms)
  â”‚   â”‚   â””â”€â”€ Stitch (Component Library â€” implementation in code)
  â”‚   â”œâ”€â”€ MotionSystem.md (Animation principles + implementation)
  â”‚   â””â”€â”€ FrontendArchitecture.md (Rendering, state, routing)
  â””â”€â”€ (All module pages, layouts, and feature UI)
```
...
The background is an infinite dark canvas (#0A0B0F). Neon accents (#6366F1, #00FFA3) are used sparingly and deliberately â€” never more than 15% of any screen. Light emits from content, not from the background.
...
Whitespace communicates relationships. Related items group together (gap 4px). Different sections separate (gap 24px). Empty space is not wasted space â€” it is the pause between thoughts.
...
|---|---|---|---|---|
| **T1 â€” Primary** | The one thing | Largest size, accent color, highest contrast | 1-3 | Page title, hero metric, primary CTA |
| **T2 â€” Secondary** | Supporting actions | Medium weight, body color, standard spacing | 5-15 | Task items, KPI values, navigation items |
| **T3 â€” Tertiary** | Reference data | Small size, secondary text color, reduced spacing | 15-50 | Timestamps, tags, metadata, descriptions |
| **T4 â€” Ambient** | System state | Smallest size, tertiary text color, optional | Unlimited | Status indicators, version info, helper text |
...
| 4 | AI insights | "Your best focus day is Tuesday" â€” synthesized intelligence |
...
Navigating away and back returns to exactly where the user was. Scroll position, active filters, selected item, search query â€” all preserved. The system never assumes "they'll find their way back."
...
|---|---|---|---|---|
| **L1 â€” Primary** | Task-completing | <2s | Immediate + state change | Complete task, save form, send message |
| **L2 â€” Secondary** | Enabling | <5s | Immediate + next state | Open modal, toggle filter, navigate |
| **L3 â€” Tertiary** | Informational | <1s | Subtle animation | Hover tooltip, scroll indicator, progress bar |
| **L4 â€” Ambient** | System | 0ms (passive) | None unless state changes | Background sync, data refresh, idle detection |
...
Action taken by user
  â†’ Is it destructive? â†’ Yes â†’ Confirmation Dialog (L1)
  â†’ Is it data-changing? â†’ Yes â†’ Toast (auto-dismiss 4s) + Undo
  â†’ Is it a system event? â†’ Yes â†’ Notification (push or in-app)
  â†’ Is it a background state? â†’ Yes â†’ Badge / Indicator
  â†’ Otherwise â†’ Inline Feedback (state change + optional tooltip)
```
...
| **Easing** | `cubic-bezier(0.4, 0, 0.2, 1)` â€” decelerate, natural feel |
...
ARIA is not a chatbot. ARIA is not a search engine. ARIA is **The Brilliant Assistant** â€” competent, anticipatory, precise, and never condescending.
...
| 3 | Interacting | Chat panel or command bar | 3/10 | User types "What's due?" â€” ARIA responds |
...
| **Urgency** | User would benefit from immediate action | "Deadline is tomorrow â€” task moved to top" |
...
| **Exploratory value** | Data-aware but not certain | "Consider moving this deadline â€” you have 5 tasks due that day" |
...
| **Learning** | Acknowledges growing understanding | "I'm learning your study patterns â€” suggestions will improve over time." |
...
**The dashboard is the user's daily cockpit â€” not a metrics museum.**
...
| Time-sensitive | "Deadline in 3 hours â€” consider prioritizing" |
| New capability discovered | "You logged 5 income entries â€” your monthly report is ready" |
| Re-engagement needed | "You haven't studied in 3 days â€” quick 15-min session?" |
...
|---|---|---|---|---|
| **L1 â€” Primary** | Sidebar / Bottom Tab | Always visible | 5-7 groups | All |
| **L2 â€” Secondary** | Navbar / Sub-tabs | Contextual | 3-5 tabs | Desktop, Tablet |
| **L3 â€” Tertiary** | Breadcrumbs | Deep pages | 2-4 levels | Desktop |
| **L4 â€” Quaternary** | In-page tabs | Module-specific | 2-5 tabs | All |
| **L5 â€” Utility** | Cmd+K Palette | On demand | Unlimited | Desktop |

### 8.3 Navigation Simplicity Rules

| Rule | Description | Example |
|---|---|---|
| **7Â±2 Groups** | Never more than 7 navigation groups in primary nav | Dashboard, Tasks, Learn, Build, Earn, Analyze, Settings |
| **Collapsible Groups** | Groups expand/collapse to show detail on demand | Learn â†’ Courses, Habits, Sleep |
...
| Briefing review | Notification â†’ 3-item summary | P0 |
| Task complete | Tap â†’ checkbox â†’ done | P0 |
| Habit log | Widget â†’ one tap per habit | P0 |
| Sleep log | Notification â†’ one-tap | P0 |
| View opportunities | Notification â†’ scroll top 3 | P1 |
...
| **Widgets** | 3 widget sizes: small (2x2 â€” 1 metric), medium (4x2 â€” briefing summary), large (4x4 â€” full quick actions) |
...
**Tablet is for review, learn, and browse â€” not for capture or deep configuration.**
...
| **Pencil support** | Handwriting â†’ AI transcription, roadmap markup |
| **Hover states** | iPadOS pointer support for desktop-like precision |
| **Drag and drop** | Drag content from browser â†’ ARIA resources |
...
| **CLI tool** | `aria task "Fix login bug"` â€” terminal-native capture |
...
**Search is the safety net for navigation â€” not the primary way to find things.**
...
| **Command search** (Cmd+K) | All commands + all data | â€” |
...
| **Fuzzy matching** | Levenshtein distance â‰¤2 for typos. Substring matching. Abbreviation expansion ("faang" â†’ "FAANG internship") |
| **Contextual ranking** | Score = relevance Ã— recency Ã— module affinity. Active module results get 2x weight |
...
```
Final Score = (TextRelevance Ã— 0.4) + (Recency Ã— 0.2) + (ModuleAffinity Ã— 0.2) + (ActionIntent Ã— 0.2)
...
|---|---|---|---|
| 1 | Direct command match | "New Task â†’" with execute button | 3 |
...
The command palette is not a search bar. It is an action launcher. Every command in the system â€” create, navigate, complete, configure, ask â€” is accessible from Cmd+K. Commands are organized by action type, not by module.
...
| **Navigate** | Move between screens | Go to Tasks, Go to Dashboard, Open Course | â†’ (blue) | Medium |
| **Complete** | Execute actions | Complete Task, Log Habit, Log Sleep | âœ“ (checkmark) | Highest |
| **Configure** | Modify settings | Settings, Customize Dashboard, Manage Profile | âš™ (gear) | Low |
| **Ask AI** | Natural language queries | "What's due?", "How did I sleep?", "Plan my week" | âœ¦ (AI icon) | Medium |
...
| **Animation** | Scale 0.95â†’1 + fade 0â†’1 (200ms) | Same | Slide up + fade (300ms) |
...
```
Trigger occurs
  â†’ Is user in focus mode? â†’ Yes â†’ Defer all non-critical
  â†’ Is it between 10 PM and 7 AM? â†’ Yes â†’ Defer all to 7 AM
  â†’ User preference: push enabled? â†’ No â†’ In-app only
  â†’ Criticality level:
      P0 Critical â†’ Push + in-app + email (if enabled)
      P1 Important â†’ Push + in-app
      P2 Informational â†’ In-app only (digest)
      P3 Background â†’ Badge only
  â†’ Deduplication check: same content sent in last 24h? â†’ Yes â†’ Suppress
  â†’ Rate check: max 5 push/day reached? â†’ Yes â†’ Downgrade to in-app
  â†’ Send
```
...
|---|---|---|---|---|---|---|
| **P0 â€” Critical** | Immediate attention | Push + in-app | 1/day | Red badge | Optional short | Deadline <1 hour, application deadline today |
| **P1 â€” Important** | Same-day value | Push + in-app | 3/day | Indigo badge | Silent | Briefing ready, weekly review ready, opportunity >90% match |
| **P2 â€” Informational** | Digest only | In-app only | 1/session | Gray badge | Silent | "3 tasks overdue", "habit streak ended", "new feature" |
| **P3 â€” Background** | Passive awareness | Badge/indicator | Unlimited | Subtle dot | Silent | System update, sync complete, feature tip |
...
| **Never notify at night** (10 PM â€” 7 AM) | Sleep hygiene. Briefing handles next morning | P0 only (deadline in <1 hour) |
...
| Weekly review | P1 | "Weekly review ready" | "72% completion â€” your best week yet!" | /weekly-review?week=current |
| Opportunity match | P1 | "New opportunity found" | "FAANG internship â€” 85% skill match" | /opportunities?id=xxx |
| Task due soon | P0 | "Task due in 30 min" | "Submit DBMS assignment â€” due today at 5 PM" | /tasks?id=xxx |
...
| Sleep wind-down | P2 | "Time to wind down" | "9:30 PM â€” log sleep for tomorrow's briefing" | /sleep |
...
```
+--------------------------------------------------+
| [Illustration â€” 120x120px, centered]              |
|                                                   |
| # Headline â€” 24px, bold (question or invitation)  |
|                                                   |
| Description â€” 14px, secondary text (2-3 lines)    |
...
| Tasks | Blank notebook + pen | "Ready for your first task?" | "Type what you need to do â€” ARIA will classify and prioritize it." | "Create a task" | "Try an example" |
...
| Goals | Empty roadmap | "What do you want to achieve?" | "Set a goal â€” career, skill, health â€” and AI will break it into milestones." | "Set a goal" | "Use a template" |
...
| Ideas | Lightbulb with spark | "Your best ideas start here" | "Capture every idea â€” no matter how rough. The good ones become projects." | "Capture an idea" | "See examples" |
...
| **Context awareness** | Empty state adapts to user type if known (Student â†’ course examples, Freelancer â†’ income examples) |
...
```
Expected duration?
  â†’ <100ms: Nothing. Instant transition. No loading state needed.
  â†’ 100ms-500ms: Skeleton. Show layout placeholder. No text.
  â†’ 500ms-2s: Skeleton + message. Show expected content shape + brief text.
  â†’ 2s-5s: Skeleton + progress. Add progress indicator + ETA if known.
  â†’ >5s: Progress + cancel. Show determinate progress bar + cancel option.
```
...
| AI generation | Skeleton + progress steps | "ARIA is analyzing..." â†’ "Finding opportunities..." stage indicator | 2-3s (Claude) / 5-10s (Ollama) |
| Image load | Blur-up | 20px blurred placeholder â†’ crossfade to full image | 500ms-2s |
| File upload | Determinate progress | Percentage bar + speed + ETA. Pause/resume support | Variable |
| Sync operation | Background badge | Small badge: "Syncing..." â†’ "Saved" with checkmark | 500ms-5s |
...
| **Animation** | Pulse: opacity 1 â†’ 0.6 â†’ 1, 1.5s ease-in-out infinite |
...
|---|---|---|---|
| **1 â€” Prevent** | Make errors impossible | Autocomplete, input constraints, format enforcement, disabled states | Every input has a max length, format constraint, and type restriction |
| **2 â€” Detect** | Catch before submission | Inline validation on blur, debounced async validation | Validate on blur + debounced async (500ms). Never validate on every keystroke |
| **3 â€” Recover** | One-click resolution | Undo (5s window), retry (3 attempts auto), revert to previous state | Every destructive action has undo. Every failed action has retry |
| **4 â€” Explain** | What happened, why, what now | Error message with tone-appropriate language + action | Every error includes cause + resolution + optional technical detail |

### 17.3 Error Tone Matrix

| Error Category | Tone | Voice Characteristic | Example |
|---|---|---|---|
| **Network** | Informative + actionable | Neutral, confident | "Couldn't save. Network issue. Saved as draft â€” will sync when connected." |
...
| **Every error has an action** | Retry, cancel, go back, or dismiss â€” never a dead end | Trivial info toasts (auto-dismiss) |
| **Errors are calm** | No red flashing, no alarming language. Neutral tone | P0 system errors may use accent-danger color |
| **Least disruptive feedback** | Inline > Tooltip > Toast > Banner > Modal â€” choose the minimum that fits severity | Choose by: blocks action? yes â†’ modal. Recoverable? yes â†’ toast. Preventable? yes â†’ inline |
...
[Title: What happened] â€” 16px, bold, tone-appropriate color
[Body: Why it happened] â€” 14px, secondary text, 1-2 sentences
[Body: What was done to recover] â€” 14px, secondary text, optional
[Action: Primary button] â€” Retry / Go Back / Dismiss
[Action: Secondary link] â€” Details / Contact Support / Try Offline Version
```
...
Easing curves follow natural physics. No linear animations â€” they feel robotic. Use `cubic-bezier(0.4, 0, 0.2, 1)` for deceleration and `cubic-bezier(0.34, 1.56, 0.64, 1)` for bounces.
...
|---|---|
| `prefers-reduced-motion` | All animations â†’ 0ms transitions |
...
**The dark mode is not a theme â€” it is the canvas.**
...
| Principle | Rationale |
|---|---|
| **Never pure black** | #000000 causes eye strain on OLED â€” use #0A0B0F as darkest |
| **Glow replaces shadow** | On dark backgrounds, shadows are invisible. Neon glow communicates depth |
| **Elevation through brightness** | Higher surfaces are lighter, not darker â€” #0A0B0F â†’ #1A1D24 |
| **Color maintains meaning** | Red still means error, green still means success â€” hue is preserved, saturation may shift |
...
|---|---|---|---|
| 0 â€” Page | `--surface-page` | #0A0B0F | Root background |
| 1 â€” Card | `--surface-card` | #13151A | Default card, containers |
| 2 â€” Elevated | `--surface-elevated` | #1A1D24 | Modals, dropdowns, popovers |
| 3 â€” Tooltip | `--surface-tooltip` | #22262E | Tooltips, toasts (highest) |
| 4 â€” Input | `--surface-input` | #0D0F14 | Form fields (darker = focus state) |
| 5 â€” Glass | `--surface-glass` | rgba(255,255,255,0.03) | Navbar, overlay backdrops |
...
**Elevation rule 19.5.2:** Do not stack more than 4 elevation levels on a single screen. Maximum z-index nesting is 4 deep: Page â†’ Card â†’ Modal â†’ Tooltip.
...
**Border rule 19.6.1:** Borders are visual hints, not structural elements. At 0.06 opacity they are barely visible by design â€” the surface itself provides structure. If a surface needs a visible border to be distinguishable, the elevation stack is wrong.
...
|---|---|---|---|---|
| Hover (card) | +4 brightness (#1A1D24 â†’ #1E2128) | none | 150ms | ease-out |
| Hover (button) | +0.05 opacity on glass | +0.1 opacity on border | 100ms | ease-out |
| Active/Press | -2 brightness (#1A1D24 â†’ #181B21) | none | 50ms | ease-in |
| Focus | none | accent border 1.5px | 0ms (instant) | â€” |
| Disabled | -0.5 opacity overlay | none | â€” | â€” |
| Selected | accent background (rgba(99,102,241,0.12)) | accent border | â€” | â€” |
...
```
Primary text:      #F1F5F9 (slate-50)   â€” 15.2:1 on page bg
Secondary text:    #94A3B8 (slate-400)  â€” 8.4:1 on page bg
Tertiary text:     #64748B (slate-500)  â€” 5.6:1 on page bg (AA minimum)
Disabled text:     #475569 (slate-600)  â€” 3.8:1 (acceptable for disabled)
Inverse text:      #0A0B0F              â€” For dark-on-light badges
```
...
| Heatmap cells | light-on-dark gradient | White â†’ accent, never dark â†’ light |
...
| Edge Case | Rule |
|---|---|
| **Loading skeleton** | Use brightness wave (whiteâ†’darkâ†’white) at 0.08 opacity base, not blue shimmer |
...
| **Focus ring** | 2px solid rgba(99,102,241,0.4) with 4px offset â€” never use outline |
| **Backdrop overlay** | rgba(0,0,0,0.6) â€” modal backdrops, drawer backdrops |
...
| **Glow on interaction only** | Animated glow effects activate on hover/focus, not static â€” reduces repaint cost |
...
```
Theme System
â”œâ”€â”€ Base tokens (invariant across themes)
â”‚   â”œâ”€â”€ Spacing (4px base Ã— 12 steps)
â”‚   â”œâ”€â”€ Typography (3 families, 11 sizes, clamp-based)
â”‚   â”œâ”€â”€ Border radius (8 values: 2-24px)
â”‚   â”œâ”€â”€ Z-index (11 levels)
â”‚   â””â”€â”€ Motion (6 durations Ã— 5 easings)
â”œâ”€â”€ Semantic tokens (theme-dependent)
â”‚   â”œâ”€â”€ Colors (background, text, border, accent at 5 surface levels)
â”‚   â”œâ”€â”€ Shadows/Glow (3 depth levels Ã— 3 accent colors)
â”‚   â”œâ”€â”€ Gradients (accent, neon, card variants)
â”‚   â””â”€â”€ Backdrop (glass opacity, blur amounts)
â”œâ”€â”€ Component tokens (theme-dependent)
â”‚   â”œâ”€â”€ Button (bg, text, border, shadow per variant Ã— state)
â”‚   â”œâ”€â”€ Card (bg, border, shadow, radius per variant)
â”‚   â”œâ”€â”€ Input (bg, border, text, placeholder per state)
â”‚   â””â”€â”€ [Every component has defined tokens]
â””â”€â”€ Pattern tokens (theme-dependent)
    â”œâ”€â”€ Gradients (accent, neon, card backgrounds)
    â”œâ”€â”€ Backgrounds (grid, noise, scanline overlays)
    â””â”€â”€ Data viz (chart series colors, grid, tooltip)
```
...
```json
{
  ...
  "tokens": {
    "base": { /* invariant â€” copied, not inherited */ },
    "semantic": {
      "colors": {
        "page": "#0A0B0F",
        "card": "#13151A",
...
**Theme format rule 20.3.1:** Every theme must define ALL semantic and component tokens. Partial themes are not allowed â€” a missing token defaults to the base theme token, which may produce visual inconsistency.
...
Base tokens (invariant)
    â””â”€â”€ Default theme (dark-cyberpunk)
            â””â”€â”€ User accent override
                    â””â”€â”€ User custom theme (extends default)
```

| Inheritance Level | Source | Overridable | Scope |
|---|---|---|---|
| L0 â€” Base | Design system | No | Application-wide |
| L1 â€” Theme | `/themes/*.theme.json` | Yes (full file) | Per-theme |
| L2 â€” Accent | User picks from 12 presets | Yes (hue rotation) | Per-user |
| L3 â€” Custom | User-provided `.theme.json` | Yes (partial merge) | Per-user |

**Inheritance rule 20.4.1:** L3 overrides L2, L2 overrides L1, L1 overrides L0. Each level only overrides tokens it explicitly defines â€” all other tokens cascade from the level below.
...
| **Contrast minimum** | textPrimary vs page â‰¥ 7:1 (AAA) | Auto-adjust or warn |
| **Contrast AA** | textSecondary vs page â‰¥ 4.5:1 (AA) | Auto-adjust or warn |
| **Accent visible** | accent vs page â‰¥ 3:1 (AA large text) | Auto-adjust |
| **No opacity leak** | All solid colors opacity = 1.0 | Reject |
| **Token count** | â‰¤ 200 tokens total | Warn if approaching |
...
| # | Name | Hue | Color | Mood |
|---|---|---|---|---|
| 1 | Indigo (default) | 239Â° | #6366F1 | Professional, focused |
| 2 | Violet | 262Â° | #8B5CF6 | Creative, imaginative |
| 3 | Pink | 336Â° | #EC4899 | Warm, energetic |
| 4 | Rose | 350Â° | #F43F5E | Passionate, urgent |
| 5 | Amber | 45Â° | #F59E0B | Warm, optimistic |
| 6 | Emerald | 160Â° | #10B981 | Growth, balance |
| 7 | Cyan | 187Â° | #06B6D4 | Technical, precise |
| 8 | Sky | 199Â° | #0EA5E9 | Open, clear |
| 9 | Fuchsia | 292Â° | #D946EF | Bold, experimental |
| 10 | Teal | 173Â° | #14B8A6 | Calm, composed |
| 11 | Orange | 24Â° | #F97316 | Dynamic, action |
| 12 | Lime | 84Â° | #84CC16 | Fresh, natural |
...
| OLED pure black | Surface tokens only (#0A0B0F â†’ #000000) | Low | v2 |
...
| Minimalist (no glow) | All glow tokens â†’ 0 opacity | Low | v3 |
...
**Accessibility is not a checklist â€” it is a design constraint built into every decision.**
...
| **Touch targets** | 44Ã—44px minimum. 8px minimum spacing between targets |
...
| **Headings** | Hierarchical heading structure (h1 â†’ h2 â†’ h3, no skipping levels) |
...
All design tokens live in `tailwind.config.js`. All component implementations exist in one place. No duplication â€” if two components look the same, they should BE the same component.
...
| **Consistent page anatomy** | Every module page follows: Header â†’ Toolbar â†’ Content â†’ Footer |
...
Every design decision today must consider how it will accommodate 50 modules, 10 platform variants, and 5 theme options â€” even if only 16 modules, 3 platforms, and 1 theme exist today.
...
| 4 | **Natural language task capture** | "Finish DBMS assignment by Friday afternoon" â†’ creates task with deadline, priority, category |
...
| 7 | **Invisible habit formation** | AI detects patterns from existing behavior and suggests habits. "You open your course app every day at 3 PM â€” turn this into a habit?" |
...
| 10 | **Time debt visualization** | "You owe yourself 2 hours of study this week." Tracks cumulative deficit vs. goal â€” honest, not guilt-tripping |

### 25.3 High-Risk, High-Reward Innovations (High Effort)

| # | Innovation | Risk | Reward |
|---|---|---|---|
| 11 | **AI study partner** | High â€” hard to get right | Very High â€” students who study with AI retain 2x more |
| 12 | **Project portfolio autogeneration** | Medium | High â€” students need portfolios for placements |
| 13 | **Peer comparison (opt-in)** | Medium â€” privacy concern | High â€” motivation through social proof |
| 14 | **Voice interface** | High â€” accuracy in Indian accents | High â€” hands-free capture, accessibility |
| 15 | **ARIA API economy** | Very High | Very High â€” extensions built by community |
...
| R2 | **Cyberpunk aesthetic polarizes users** | Medium | Medium | Refined cyberpunk â€” professional edge, not abrasive. Test with users |
| R3 | **AI ghost mode is too invisible â€” users don't know what's AI** | High | High | Progressive AI education: ghost â†’ suggesting â†’ explaining as user explores |
...
|---|---|---|
| R1 â€” Dark mode rejection | User surveys, support tickets | Quarterly |
| R2 â€” Aesthetic polarization | User testing, design feedback | Per feature release |
| R3 â€” AI invisibility | User interviews, feature adoption metrics | Monthly |
| R4 â€” Module overload | Onboarding funnel analysis, module adoption | Weekly first month |
| R5 â€” Sync conflicts | Error reporting, support tickets | Weekly |
| R7 â€” Accessibility gaps | axe-core CI results, audit findings | Per sprint |
| R8 â€” Design system drift | Visual regression testing, code review | Per PR |
...
**What:** Perfect the briefing UX before adding new modules. The briefing should feel like the product's daily "hello" â€” warm, intelligent, and indispensable.

#### R2: Design the Return Flow Before the Happy Path
Most design effort goes into the first-time user experience. But 60% of sessions are return visits after 24h+ absence. The return flow â€” catch-up digest, guilt-free reset, context restoration â€” is the highest-leverage design investment.
...
Before implementing specific AI features, define the AI design language â€” ghost/invoke model, 4 visibility levels, intervention thresholds, suggestion rules, feedback templates. Every AI feature uses the same patterns.
...
Cmd+K is not a search bar â€” it is the fastest path to every action in the system. As the system grows from 16 to 50+ modules, the command palette becomes the primary navigation method for power users.
...
**Palette rule 28.4.1:** All UI colors are derived from these 5 hue families. New colors must map to an existing family â€” no ad-hoc hues.
...
|---|---|---|---|
| `--color-bg-page` | --raw-slate-950 | Root background | â€” |
| `--color-bg-card` | --raw-slate-900 | Card, container bg | â€” |
| `--color-bg-elevated` | --raw-slate-800 | Modal, dropdown bg | â€” |
...
| `--color-accent-soft` | rgba(99,102,241,0.12) | Selected bg, active bg | â€” |
...
| `--color-border-default` | rgba(255,255,255,0.06) | Card borders | â€” |
| `--color-border-accent` | rgba(99,102,241,0.3) | Focus ring, active tab | â€” |

**Semantic mapping rule 28.5.1:** Each semantic token maps to exactly one raw value. No calculation, no concatenation â€” the token IS the value.
...
**Font selection rule 29.2.1:** Syne is for display only â€” never use Syne for body text or UI labels. Its expressive letterforms sacrifice readability at small sizes.
...
| **Loading skeleton text** | No text rendered â€” use skeleton shapes at same dimensions as final text |
...
| **Tree-shaking** | Import individual icons by name â€” never bundle the full library |
| **Custom icons** | Must match Lucide grid geometry (24Ã—24, 2px stroke, rounded caps/joins) |
...
|---|---|---|
| `--icon-xs` | 12Ã—12 | Inline with text, badge indicators |
| `--icon-sm` | 16Ã—16 | Table cell actions, compact toolbars |
| `--icon-md` | 20Ã—20 | Button icons (with text), list item icons |
| `--icon-lg` | 24Ã—24 | Standalone icon buttons, empty state illustrations |
| `--icon-xl` | 32Ã—32 | Navigation icons, module icons |
| `--icon-2xl` | 40Ã—40 | Feature icons, dashboard KPIs |
| `--icon-3xl` | 48Ã—48 | Empty state primary icon, onboarding steps |

**Icon size rule 30.3.1:** Icons inside buttons scale with button size: sm button â†’ icon-sm, md button â†’ icon-md, lg button â†’ icon-lg. Never use a different size.
...
| **Padding** | 2px inset from edges (content in 20Ã—20 safe area) |
| **Negative space** | Minimum 1px gap between elements |
| **Filled areas** | Use fill for status indicators, outline for functional icons |
| **Animation ready** | Design with motion in mind â€” separate elements into distinct paths/groups |

### 30.6 Icon Accessibility

| Requirement | Implementation |
|---|---|
| **Every icon has aria-label** | `<Icon aria-label="Add task" />` â€” descriptive, not "icon" |
| **Decorative icons have aria-hidden** | `<Icon aria-hidden />` â€” icons purely visual |
...
| **Touch target** | Icon buttons minimum 44Ã—44px hit area (padding included) |
...
| **Heart / star toggle** | Favorites | Scale pulse (1â†’1.2â†’1, 200ms) |
| **Notification bell** | New notification | Shake (translateX Â±3px, 3 cycles, 400ms) |
| **AI processing** | AI is thinking | Pulsing glow on icon (opacity 0.6â†’1, 1s, ease-in-out) |
...
**Components are design decisions frozen in code.** Every component codifies a design rule â€” if the rule changes, the component changes. This prevents design drift across 16+ modules.
...
<App>
  <Layout>
    <Navbar />        â€” Global navigation, search, user menu
    <Sidebar />       â€” Module navigation, pinned items
    <Main>
      <PageHeader />  â€” Page title, actions, breadcrumbs
      <Toolbar />     â€” Module-specific actions, filters
      <Content />     â€” Module-specific content area
      <PageFooter />  â€” Pagination, timestamps, metadata
    </Main>
  </Layout>
</App>
```
...
**Category rule 31.3.1:** Primitives and Layout components cannot import from Module or Pattern components. Dependencies flow one direction: Module â†’ Composite â†’ Primitive.
...
**Prop rule 31.4.1:** Every component must accept `className` and forward it to the outermost DOM node. This is non-negotiable â€” it enables one-off overrides without forking the component.
...
| **Checkbox** | Binary choices, multiple selection | 18Ã—18 hit area 44Ã—44 |
| **Radio** | Single choice from small set | 18Ã—18 hit area 44Ã—44 |
| **Toggle** | On/off settings | 36Ã—20 track, 16Ã—16 thumb |
...
| **Async (availability)** | 300ms after last keystroke | Debounced API call | Pulsing border â†’ success/error |
...
| **Debounce timing** | â€” | 300ms for text, 150ms for select | â€” |
...
| **Required indicator** | `(required)` text or `*` with `aria-hidden` â€” but text preferred |
...
| **Tab order** | Natural DOM order matches visual order â€” no tabindex manipulation |
...
| **Smart fill** | User clicks "Auto-fill" button | AI populates multiple fields from context (e.g., "Meeting notes" â†’ title, tags, project) |
...
â”œâ”€â”€ "What is the trend over time?"
â”‚   â†’ Line chart (continuous data, 5+ points)
â”‚   â†’ Area chart (cumulative or volume emphasis)
â”‚   â†’ Sparkline (compact, single metric, <200px wide)
â”œâ”€â”€ "How do categories compare?"
â”‚   â†’ Bar chart (vertical, <12 categories)
â”‚   â†’ Horizontal bar (12+ categories or long labels)
â”‚   â†’ Donut chart (3-6 segments, percentage of whole only)
â”œâ”€â”€ "What is the distribution/composition?"
â”‚   â†’ Stacked bar (parts of whole over time)
â”‚   â†’ Donut chart (single time period, parts of whole)
â”‚   â†’ Histogram (frequency distribution)
â”œâ”€â”€ "What is the relationship between variables?"
â”‚   â†’ Scatter plot (2 variables, <1000 points)
â”‚   â†’ Bubble chart (3 variables, <500 points)
â”‚   â†’ Heatmap (2 categorical variables)
â”œâ”€â”€ "What is my progress toward a goal?"
â”‚   â†’ Gauge / radial progress (<200Â° arc)
â”‚   â†’ Progress bar (linear, compact)
â”‚   â†’ Bullet chart (actual vs target vs range)
â””â”€â”€ "What should I do next?"
    â†’ No chart needed â€” use a ranked list or action prompt
```
...
| **Axes** | No visible axis lines â€” only grid lines |
...
|---|---|
| **<300px** | Sparkline or single KPI number â€” no full chart |
...
| **Click datapoint** | Drill into detail (day â†’ hour breakdown) | Transition to detail view within same component slot |
...
| **Export** | Right-click â†’ "Copy as image" or "Download CSV" | Context menu on chart container |

### 33.6 Performance for Large Datasets

| Data Size | Rendering Strategy | Max Points |
|---|---|---|
| **Small (<100 points)** | SVG â€” full fidelity, all points interactive | â€” |
| **Medium (100-1000 points)** | SVG with data decimation (LTTB algorithm) | â€” |
| **Large (1000-10000 points)** | Canvas rendering via HTML5 Canvas | â€” |
| **Very large (10000+)** | Canvas + time-series bucketing (aggregate by minute/hour/day) | â€” |
...
**Responsive is not about fitting screens â€” it is about preserving capability.** Every responsive breakpoint should maintain the same user capabilities, not remove them. A mobile user should be able to accomplish anything a desktop user can â€” just in a different layout.
...
|---|---|---|
| **Stack** | Multi-column â†’ single column on mobile | Dashboard grid â†’ stacked cards |
...
| **Transform** | Component changes form at breakpoints | Table â†’ Card list on mobile |
...
| **Pagination** | Infinite scroll on mobile â†’ paginated on desktop | Search results, activity feed |

**Responsive pattern rule 34.3.1:** "Hide" pattern is reserved for non-critical content only. Navigation, primary actions, and content must never be hidden â€” only reorganized.

**Responsive pattern rule 34.3.2:** "Transform" pattern (table â†’ card) requires identical data in both formats. Users must not lose column visibility when switching to cards.
...
```
Module: Dashboard
â”œâ”€â”€ P0 (always visible)
â”‚   â”œâ”€â”€ Daily briefing summary
â”‚   â”œâ”€â”€ Next task (single)
â”‚   â””â”€â”€ One KPI
â”œâ”€â”€ P1 (visible on mobile, more detail on desktop)
â”‚   â”œâ”€â”€ Task list (3 items mobile â†’ 10 items desktop)
â”‚   â”œâ”€â”€ Habit streak
â”‚   â””â”€â”€ Quick capture
â””â”€â”€ P2 (desktop only, accessible via navigation on mobile)
    â”œâ”€â”€ Opportunity radar
...
|---|---|---|
| **Hit targets** | Minimum 44Ã—44px | Minimum 24Ã—24px |
...
**Interaction rule 34.5.2:** Touch targets for primary actions must be 48Ã—48px minimum. Secondary actions (icon buttons) minimum 44Ã—44px.
...
| **Charts** | SVG renders as vector â€” high quality print |
...
|---|---|---|
| **Dashboard** | SSR â†’ Hydrate â†’ Client fetch for real-time data | Initial view must be instant. Real-time data updates via client fetch after hydrate |
| **List pages** (tasks, courses, habits) | SSR with streaming (skeleton â†’ content) | Users scan lists quickly. Show structure first, content fills in |
...
| **Login / auth** | SSR (critical â€” non-negotiable) | Must render before any JS loads. Forms must work without JS |
...
| **Font (Syne variable)** | 30KB | â€” | â€” |
| **Font (DM Sans variable)** | 25KB | â€” | â€” |
| **Font (JetBrains Mono)** | 15KB | â€” | â€” |
...
| **Avatar** | WebP | 96Ã—96 (2x) | Eager | 1 year |
| **OG images** | JPEG | 1200Ã—630 | â€” | Edge cache |
...
**Font rule 35.5.1:** Preload Syne in the document `<head>` â€” headings are critical for first paint. DM Sans uses preconnect (lighter than preload, still reduces latency).
...
| **Colors (bg, text, border)** | CSS transition | No (layout/paint) â€” use sparingly |
| **Width / height** | CSS transition | No (layout) â€” never animate |
| **Box-shadow / glow** | CSS transition | No (paint) â€” limit to 1-2 elements |
...
**Memory rule 35.7.1:** The application must maintain stable memory usage during a typical 8-hour session. Measure memory before and after each feature â€” if memory grows >10% after the feature, the feature needs optimization.
...
*This document is the design constitution. Every design decision, component specification, and interaction pattern must trace back to a principle or strategy defined here. If a decision conflicts with this document, revisit the decision â€” not the document.*
