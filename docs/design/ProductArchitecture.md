# Product Architecture â€” Second Brain OS (ARIA)
...
| AI Architecture | In-process async agents (per ADR-004) with dual backend (Ollama â†’ Claude â†’ Algorithmic) |
...
1. **Developer experience** â€” fast iteration, clear boundaries, testability
2. **Graceful degradation** â€” every feature works without AI, without network, without backend
3. **Event-driven coupling** â€” domains communicate through events, not imports
4. **Fail-safe defaults** â€” every agent falls back to algorithmic mode if the LLM is unreachable
5. **Offline-first** â€” the app must be usable without internet for core CRUD operations
...
1. **Full AI** â€” All agents online, LLM generating insights
2. **Algorithmic** â€” AI unavailable, deterministic fallback logic
3. **Offline** â€” No network, cached data, queued writes
...
```
LLM (primary) â†’ Algorithmic fallback â†’ Static default response
```
...
2. If Ollama fails â†’ try Claude (cloud, ~$0.015/req)
3. If both fail â†’ algorithmic fallback (deterministic logic)
4. If all fail â†’ static default response
...
**Status:** Extended â€” No change needed.

### ADR-006: APScheduler over Celery (Confirmed)
**Status:** Extended â€” No change needed.

### ADR-007: PWA over Native Mobile (Confirmed)
**Status:** Extended â€” Offline-first architecture enforced in all frontend patterns.

### ADR-008: No Event Bus in Alpha (Extended for Gamma)
**Status:** Extended
**Decision (Alpha):** In-process function calls + Supabase Realtime + Cron polling.
**Decision (Beta/Gamma):** Introduce `asyncio.Queue`-based in-process event bus for beta, RabbitMQ for gamma. This extension does not change the alpha implementation. The event bus abstraction layer is defined now; the implementation evolves from direct calls â†’ in-process queue â†’ dedicated broker without changing event producers or consumers.
...
- **Purpose** â€” Why this domain exists
- **Responsibilities** â€” What it owns
- **Data Ownership** â€” Tables and schemas it controls
- **Inputs** â€” Events and API calls it accepts
- **Outputs** â€” Events it publishes
- **Relationships** â€” How it connects to other domains
- **User Interactions** â€” What users do in this domain
- **AI Interactions** â€” How AI agents interact with this domain
...
#### Purpose
The Knowledge domain manages the user's personal knowledge base â€” a structured, searchable, AI-enriched collection of information. It goes beyond simple notes to support rich content types, embeddings, and semantic relationships.
...
#### Purpose
Goals provide strategic direction for the user's daily actions. The Goal domain manages goal hierarchies (high-level aspirations â†’ measurable objectives â†’ daily tasks), progress tracking, and milestone management.
...
#### User Interactions
- Goal creation wizard (aspiration â†’ measurable â†’ timeline)
...
#### Purpose
The Opportunity domain scans the user's context (skills, interests, goals) and matches them against external opportunities â€” jobs, internships, hackathons, scholarships, open-source projects, networking events. It's the "career radar" that proactively finds relevant openings.
...
#### Responsibilities
- ARIA orchestrator â€” intent classification, agent dispatch, response synthesis
- Agent management â€” 15 agents with lifecycle, fallback, and monitoring
- Prompt management â€” YAML-frontmatter prompt files via PromptLoader
- LLM client management â€” Ollama (primary), Claude (fallback), algorithmic (last resort)
...
#### AI Interactions
- **ARIA Orchestrator:** Central dispatcher â€” receives all user messages, classifies, dispatches to sub-agents
...
#### Purpose
The Search domain provides unified search across all content types â€” tasks, knowledge items, goals, projects, opportunities, chat history. It combines full-text search with semantic (vector) search for hybrid ranking.
...
#### Purpose
The Memory domain provides persistent, structured storage for AI agent memory. Unlike chat history (raw conversation), memory stores extracted knowledge about the user â€” preferences, patterns, facts, and long-term context â€” with confidence scoring, decay, and provenance tracking.
...
| Domain | Relationship |
|---|---|
| AI (D09) | Primary consumer â€” all agents read/write memory |
...
```
User Domain (D10) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    â”‚                                                               â”‚
    â–¼                                                               â”‚
Task Domain (D01) â—„â”€â”€â–º Goal Domain (D04) â—„â”€â”€â–º Roadmap Domain (D05) â”‚
    â”‚                     â”‚                        â”‚                â”‚
    â”‚                     â–¼                        â–¼                â”‚
    â”‚              Project Domain (D07) â—„â”€â”€â–º Opportunity Domain     â”‚
    â”‚                     â”‚                        (D06)            â”‚
    â–¼                     â–¼                                         â”‚
Knowledge Domain (D02) â—„â”€â”€â–º Learning Domain (D03)                   â”‚
    â”‚                     â”‚                                         â”‚
    â–¼                     â–¼                                         â”‚
Search Domain (D12) â—„â”€â”€â–º Memory Domain (D13)                        â”‚
    â”‚                     â”‚                                         â”‚
    â–¼                     â–¼                                         â”‚
Notification Domain (D11) â—„â”€â”€â–º AI Domain (D09) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                                    â”‚
                                    â–¼
                           Analytics Domain (D08) â€” subscribes to ALL events
```
...
| Opportunity (D06) | Learning, Goal | Self | 4 | â€” |
| Project (D07) | Goal, Roadmap | Self | 5 | â€” |
| Analytics (D08) | All domains | Self | 3 | ALL events |
| AI (D09) | All domains | Self, Memory | 4 | All domain events |
| User (D10) | Self | Self | 4 | â€” |
...
```
task.completed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚                                                       â”‚
    â”œâ”€â”€â–º Goal.progress_updated                              â”‚
    â”œâ”€â”€â–º Analytics.metric_computed                          â”‚
    â”œâ”€â”€â–º Notification (optional congrats)                  â”‚
    â”œâ”€â”€â–º Memory (pattern stored)                            â”‚
    â””â”€â”€â–º Learning (if task relates to course)              â”‚
                                                           â”‚
goal.at_risk â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
    â”œâ”€â”€â–º Notification (alert user)                          â”‚
    â”œâ”€â”€â–º AI (Planner suggests corrective tasks)             â”‚
    â””â”€â”€â–º Analytics (risk trend detected)                    â”‚
                                                           â”‚
knowledge.item_created â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
    â”œâ”€â”€â–º Search.index_updated                               â”‚
    â”œâ”€â”€â–º Learning (potential gap analysis trigger)          â”‚
    â””â”€â”€â–º Memory (knowledge seed)                            â”‚
                                                           â”‚
opportunity.detected â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
    â”œâ”€â”€â–º Notification (high match â†’ alert)                  â”‚
    â”œâ”€â”€â–º Analytics (opportunity funnel tracked)             â”‚
    â””â”€â”€â–º Memory (preference reinforced/updated)             â”‚
                                                           â”‚
ai.agent_completed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
    â”œâ”€â”€â–º Analytics (latency, tokens, fallback tracked)      â”‚
    â”œâ”€â”€â–º Memory (if agent produced insights)                â”‚
    â””â”€â”€â–º Notification (if agent had user-facing output)     â”‚
```
...
|---|---|---|
| Event publish | Domain â†’ Event Bus | Task publishes `task.completed` |
| Event subscribe | Event Bus â†’ Domain | Goal subscribes to `task.completed` |
| Query API | Domain â†’ Domain API | Frontend calls Goal API from Task page |
| Shared infrastructure | All â†’ Supabase | Each domain uses its own schema |
| AI orchestration | AI â†’ All | ARIA calls agent functions across domains |
...
Unified Search                                                             â—
Memory Management                                                             â—
Briefing Generation       â—‹       â—‹       â—‹    â—‹    â—    â—‹    â—‹    â—‹    â—‹
Weekly Review             â—‹       â—‹    â—‹         â—‹    â—         â—‹    â—‹
Habit Tracking       â—
Sleep Tracking             â—
Idea Pipeline             â—‹                                     â—
Income Tracking                                                      â—
...
**â— = Primary / ðŸ”µ = Supporting / â—‹ = Consumes**
...
```
apps/web (Next.js) â”€â”€â”€â”€â”€â”€â”€â–º apps/api (FastAPI) â”€â”€â”€â”€â”€â”€â”€â–º packages/ai (Agents)
     â”‚                                                      â”‚
     â”‚                                                      â–¼
     â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º packages/config â”€â”€â”€â”€â”€â”€â”€â–º packages/database
                                      â”‚
                                      â–¼
...
```
Morning (7:00 AM)
  â”œâ”€â”€ Push notification: Daily Briefing ready
  â”œâ”€â”€ User opens app â†’ sees briefing summary
  â”‚     â”œâ”€â”€ Today's tasks (from Task Domain)
  â”‚     â”œâ”€â”€ Goal progress (from Goal Domain)
  â”‚     â”œâ”€â”€ Learning streak (from Learning Domain)
  â”‚     â””â”€â”€ Opportunity match (from Opportunity Domain)
  â””â”€â”€ User reviews, adjusts task priorities

Daytime (8:00 AM - 6:00 PM)
  â”œâ”€â”€ Task work
  â”‚     â”œâ”€â”€ Create/complete tasks
  â”‚     â”œâ”€â”€ Task dependencies managed
  â”‚     â””â”€â”€ Time tracking (start/stop timer)
  â”œâ”€â”€ Learning
  â”‚     â”œâ”€â”€ Study session with Pomodoro timer
  â”‚     â”œâ”€â”€ Course progress logged
  â”‚     â””â”€â”€ Knowledge items captured
  â”œâ”€â”€ Periodic nudges
  â”‚     â”œâ”€â”€ 6 PM: Course nudge (if coursework pending)
  â”‚     â””â”€â”€ Missed habit reminder (if applicable)
  â””â”€â”€ Opportunity radar (6 AM automated scan)
        â””â”€â”€ Notification if high-match opportunity found

Evening (9:30 PM)
  â”œâ”€â”€ Sleep wind-down notification
  â”œâ”€â”€ Sleep log entry
  â””â”€â”€ Day summary in notification center

Weekend (Sunday 8 PM)
  â””â”€â”€ Weekly Review delivered as digest
```

### 9.2 Core User Journeys

#### Journey 1: Task Completion â†’ Goal Progress â†’ Briefing Update
```
User completes task â”€â”€â–º Task Domain publishes task.completed
    â”‚
    â”œâ”€â”€â–º Goal Domain receives event
    â”‚     â””â”€â”€â–º Recalculates goal progress_pct
    â”‚     â””â”€â”€â–º If milestone reached: publishes goal.milestone_reached
    â”‚
    â”œâ”€â”€â–º Notification Domain
    â”‚     â””â”€â”€â–º Sends congrats notification (if enabled)
    â”‚
    â”œâ”€â”€â–º Analytics Domain
    â”‚     â””â”€â”€â–º Updates productivity metrics
    â”‚
    â”œâ”€â”€â–º Memory Domain
    â”‚     â””â”€â”€â–º Stores pattern: "user completes tasks by priority"
    â”‚
    â””â”€â”€â–º AI Domain (next briefing)
          â””â”€â”€â–º Daily Briefing includes updated goal progress
```

#### Journey 2: Knowledge Capture â†’ Idea Pipeline â†’ Project
```
User saves web article â”€â”€â–º Knowledge Domain creates item
    â”‚
    â”œâ”€â”€â–º Search Domain re-indexes
    â”‚
    â”œâ”€â”€â–º Learning Domain (if article relates to course)
    â”‚     â””â”€â”€â–º Updates gap analysis
    â”‚
    â””â”€â”€â–º User later promotes to Project
          â””â”€â”€â–º Project Domain creates from knowledge item
          â””â”€â”€â–º Task Domain creates initial tasks
          â””â”€â”€â–º Notification: project created
```

#### Journey 3: Opportunity Detection â†’ Application â†’ Career Goal Progress
```
Radar Agent (6 AM) â”€â”€â–º Opportunity Domain
    â”‚
    â”œâ”€â”€â–º Scores new opportunities against user profile
    â”œâ”€â”€â–º Publishes opportunity.detected (match_score >= 0.7)
    â”‚
    â”œâ”€â”€â–º Notification: "New opportunity: Senior Dev at X (92% match)"
    â”‚
    â””â”€â”€â–º User applies
          â””â”€â”€â–º Opportunity.applied event
          â””â”€â”€â–º Goal progress updated (if career goal linked)
          â””â”€â”€â–º Analytics tracks application funnel
```
...
Frontend (Next.js)                  Backend (FastAPI)                     Database (Supabase)
       ...
       â”‚                              [Validate Input]                           â”‚
       â”‚                                  â”‚                                      â”‚
       â”‚                              [Supabase Insert] â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º    â”‚
       â”‚                                  â”‚                    INSERT tasks      â”‚
       ...
       â”‚                              task.created                               â”‚
       â”‚                                  â”‚                                      â”‚
       â”‚                                  â”‚â—„â”€â”€ Event Bus â”€â”€â–º Domain Consumers    â”‚
       â”‚                                  â”‚                                      â”‚
       â”‚  {task_id, title, ...}           â”‚                                      â”‚
...
Producer Domain                    Event Bus                        Consumer Domain(s)
       â”‚                              â”‚                                    â”‚
       â”‚  Publish: task.completed     â”‚                                    â”‚
       â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º      â”‚                                    â”‚
       â”‚                              â”‚                                    â”‚
       â”‚                              â”‚  Dispatch to subscribers           â”‚
       â”‚                              â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º Goal       â”‚
       â”‚                              â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º Analytics  â”‚
       â”‚                              â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º Memory     â”‚
       â”‚                              â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º Notificationâ”‚
       â”‚                              â”‚                                    â”‚
       â”‚                              â”‚  Each consumer acknowledges        â”‚
...
[Scheduler cron]                     [ARIA Orchestrator]                  [Agent Module]
   â”‚  7 AM trigger briefing              â”‚        â”‚                     packages/ai/agents/
   â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º         â”‚        â”‚                           â”‚
                                         â”‚  Classify: /briefing              â”‚
                                         â”‚  Load prompt                      â”‚
                                         â”‚   â””â”€â”€ PromptLoader                â”‚
                                         â”‚        â””â”€â”€ prompts/agents/        â”‚
                                         â”‚             briefing_agent.md     â”‚
                                         â”‚                                   â”‚
                                         â”‚  Dispatch to Briefing Agent â”€â”€â–º   â”‚
                                         â”‚                                   â”‚
                                         â”‚                              [Build Context]
                                         â”‚                               â”œâ”€â”€ Tasks (API)
                                         â”‚                               â”œâ”€â”€ Goals (API)
                                         â”‚                               â”œâ”€â”€ Learning (API)
                                         â”‚                               â”œâ”€â”€ Analytics (API)
                                         â”‚                               â””â”€â”€ Memory (API)
                                         â”‚                                   â”‚
                                         â”‚                              [Call LLM]
                                         â”‚                               â”œâ”€â”€ Ollama (try)
                                         â”‚                               â”œâ”€â”€ Claude (fallback)
                                         â”‚                               â””â”€â”€ Algorithmic (last)
                                         â”‚                                   â”‚
                                         â”‚  â—„â”€â”€â”€â”€ Briefing result â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ â”‚
                                         â”‚                                   â”‚
                                         â”‚  [Publish result]                 â”‚
                                         â”‚   â”œâ”€â”€ Notification (deliver)      â”‚
                                         â”‚   â”œâ”€â”€ Memory (store pattern)      â”‚
                                         â”‚   â””â”€â”€ Analytics (log run)         â”‚
```
...
| Store Memory | â€” | Memory table | `memory.stored` |
...
- Add fields only (backward compatible) â†’ increment minor version
- Remove/rename fields (breaking) â†’ new event type, old type deprecated but still published for one cycle
...
Content Created/Updated (any domain)
    â”‚
    â”œâ”€â”€â–º Search Event Bus subscriber receives domain event
    â”‚
    â”œâ”€â”€â–º Extracts title, content, metadata from entity
    â”‚
    â”œâ”€â”€â–º Generates embedding (via local embedding model or API)
    â”‚
    â”œâ”€â”€â–º Updates search.documents table:
    â”‚     - tsvector: to_tsvector('english', title || ' ' || coalesce(content, ''))
    â”‚     - embedding: pgvector column
    â”‚     - metadata: domain, entity_id, created_at
    â”‚
    â””â”€â”€â–º Publishes search.index_updated event
```
...
User Settings (users.settings)
    â””â”€â”€ notification_preferences.json (JSONB)
        â”œâ”€â”€ Channel-level toggles
        â”‚     â”œâ”€â”€ in_app: true/false
        â”‚     â”œâ”€â”€ push: true/false
        â”‚     â”œâ”€â”€ email: true/false (with digest_frequency: immediate/daily/weekly)
        â”‚     â””â”€â”€ sms: true/false
        â”‚
        â”œâ”€â”€ Category-level toggles
        â”‚     â”œâ”€â”€ task_reminders: {in_app: true, push: true}
        â”‚     â”œâ”€â”€ goal_milestones: {in_app: true, email: true}
        â”‚     â”œâ”€â”€ opportunity_alerts: {push: true}
        â”‚     â””â”€â”€ learning_nudges: {in_app: true, push: false}
        â”‚
        â””â”€â”€ Quiet hours
              â”œâ”€â”€ enabled: true
              â”œâ”€â”€ start: "22:00"
...
Notification Domain subscriber receives event
    â”‚
    â”œâ”€â”€â–º Check user preferences for this event type
    â”‚     â””â”€â”€â–º Skip if user disabled
    â”‚
    â”œâ”€â”€â–º Check quiet hours
    â”‚     â””â”€â”€â–º Queue for delivery after quiet hours if non-critical
    â”‚
    â”œâ”€â”€â–º Create notification record
    â”‚     â””â”€â”€â–º INSERT INTO notifications.notifications (...)
    â”‚
    â”œâ”€â”€â–º Dispatch to channels
    â”‚     â”œâ”€â”€â–º In-app: ready for frontend to poll
    â”‚     â”œâ”€â”€â–º Push: send via Service Worker
    â”‚     â””â”€â”€â–º Email: add to digest queue (if digest mode)
    â”‚
    â””â”€â”€â–º Publish notification.sent event
```
...
CREATION (memory.stored)
    â”‚
    â”œâ”€â”€ Confidence: 0.7 (default for single observation)
    â”‚
    â–¼
CONSOLIDATION (runs daily via Memory Agent)
    â”‚
    â”œâ”€â”€ Merge duplicates: if 2+ memories with similar embedding and same type:
    â”‚     â”œâ”€â”€ Keep highest confidence
    â”‚     â”œâ”€â”€ Merge context_tags
    â”‚     â””â”€â”€ Update source_agent to "memory_agent"
    â”‚
    â”œâ”€â”€ Resolve conflicts: if contradictory memories exist:
    â”‚     â”œâ”€â”€ Higher confidence wins
    â”‚     â”œâ”€â”€ Equal confidence â†’ newer wins
    â”‚     â””â”€â”€ Old/low-confidence memory gets flagged for review
    â”‚
    â–¼
DECAY (runs hourly)
    â”‚
    â”œâ”€â”€ For each memory not accessed in >7 days:
    â”‚     â”œâ”€â”€ confidence *= (1 - decay_rate)
    â”‚     â””â”€â”€ If confidence < 0.1 â†’ archive or delete
    â”‚
    â–¼
RETRIEVAL (on AI context building)
    â”‚
    â”œâ”€â”€ Query by user_id + context_tags
    â”œâ”€â”€ Semantic similarity search on embedding
    â”œâ”€â”€ Filter by confidence > 0.3
    â”œâ”€â”€ Sort by confidence * recency_boost
    â”‚     â””â”€â”€ recency_boost = 1.0 / (1.0 + days_since_last_access)
    â””â”€â”€ Return top-K memories (configurable, default K=20)
```
...
class ARIAOrchestrator:
    ...
    async def process_message(self, message: str, user_id: str) -> str:
        ...
        # Step 4: Default â€” use ARIA system prompt with full context
...
| A01 | Planner | â€” | 7 AM + on-demand | Yes | Design |
| A02 | Memory | `agents/memory_agent.md` | Every chat (bg) | Yes | âœ… Live |
| A03 | Learning | `agents/learning_agent.md` | Daily + on-demand | Yes | âœ… Live |
| A04 | Reminder | â€” | Every 15 min | No | âœ… Live |
| A05 | Career | â€” | Weekly + on-demand | Yes | Design |
| A06 | Opportunity | `agents/opportunity_radar_agent.md` | 6 AM daily | Yes | âœ… Live |
| A07 | Analytics | â€” | Real-time + weekly | No | Design |
| A08 | Roadmap | â€” | On-demand + weekly | Yes | Design |
| A09 | Daily Briefing | `agents/briefing_agent.md` | 7 AM daily | Yes | âœ… Live |
| A10 | Weekly Review | `agents/weekly_review_agent.md` | Sun 8 PM | Yes | âœ… Live |
| A11 | Missed Task Checker | â€” | Every 15 min | No | âœ… Live |
| A12 | Habit Miss Checker | â€” | Midnight daily | No | âœ… Live |
| A13 | Sleep & Bedtime | `agents/sleep_agent.md` | 9:30 PM + wake | Yes | âœ… Live |
| A14 | Course Nudge | `agents/nudge_agent.md` | 6 PM daily | Yes | âœ… Live |
...
Event Ingestor (validates, enriches, stores in event_log)
    â”‚
    â”œâ”€â”€â–º Stream Processor (real-time: running counters, moving averages)
    â”‚     â””â”€â”€â–º Redis (or in-memory dict) for hot counters
    â”‚
    â”œâ”€â”€â–º Batch Processor (every 15 min: compute metrics)
    â”‚     â””â”€â”€â–º analytics.metrics upsert
    â”‚
    â”œâ”€â”€â–º Trend Detector (every hour: detect significant movements)
    â”‚     â””â”€â”€â–º analytics.trends insert
    â”‚
    â””â”€â”€â–º Data Retention (daily: prune events >90 days, aggregate to hourly)
```
...
Analytics Domain collects data from multiple sources:
    â”œâ”€â”€â–º Task stats: supabase.from_("tasks.tasks").select("count(*)").eq("user_id", uid)...
    â”œâ”€â”€â–º Goal stats: supabase.from_("goals.goals").select("progress_pct, count").eq("user_id", uid)...
    â”œâ”€â”€â–º Learning stats: supabase.from_("learning.progress").select("*").eq("user_id", uid)...
    â”œâ”€â”€â–º Habit stats: supabase.from_("habits.habit_logs").select("*").eq("user_id", uid)...
    â”œâ”€â”€â–º Metrics: supabase.from_("analytics.metrics").select("*").eq("user_id", uid)...
    â””â”€â”€â–º Trends: supabase.from_("analytics.trends").select("*").eq("user_id", uid)...
    â”‚
    â–¼
...
Online Mode
    â”‚
    â”œâ”€â”€â–º Read: Fetch from API, cache in IndexedDB
    â”œâ”€â”€â–º Write: Send to API, update cache on success
    â””â”€â”€â–º Realtime: WebSocket updates pushed from backend

Offline Mode
    â”‚
    â”œâ”€â”€â–º Read: Serve from IndexedDB cache
    â”œâ”€â”€â–º Write: Queue to IndexedDB sync queue
    â””â”€â”€â–º Realtime: Deferred until reconnection

Reconnection
    â”‚
    â”œâ”€â”€â–º Flush sync queue (ordered by created_at)
    â”œâ”€â”€â–º Conflict resolution: last-write-wins (with notification if data lost)
    â””â”€â”€â–º Refresh caches from API
```
...
â”‚        Status Bar            â”‚  â† safe-area-inset-top
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
...
â”‚   - Bottom sheets for forms  â”‚
â”‚                              â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚   Bottom Nav (fixed, 64px)   â”‚  â† safe-area-inset-bottom
...
â”‚  Title Bar (OS native or custom)                    â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚        â”‚                                            â”‚
â”‚ Sidebarâ”‚         Main Content Area                  â”‚
â”‚ (240px)â”‚         (flexible)                         â”‚
â”‚        â”‚                                            â”‚
â”‚  ðŸ“‹    â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”  â”‚
â”‚  ðŸŽ¯    â”‚  â”‚  Task List      â”‚  â”‚  Task Detail     â”‚  â”‚
â”‚  ðŸ“š    â”‚  â”‚  (scrollable)   â”‚  â”‚  (scrollable)    â”‚  â”‚
â”‚  ðŸ’¡    â”‚  â”‚                 â”‚  â”‚                  â”‚  â”‚
â”‚  ðŸ”    â”‚  â”‚  Task 1         â”‚  â”‚  Title: ...      â”‚  â”‚
â”‚  âš™ï¸    â”‚  â”‚  Task 2         â”‚  â”‚  Status: ...     â”‚  â”‚
â”‚        â”‚  â”‚  Task 3         â”‚  â”‚  Priority: ...   â”‚  â”‚
â”‚        â”‚  â”‚                 â”‚  â”‚  Due: ...        â”‚  â”‚
â”‚        â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â”‚
â”‚        â”‚                                            â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚  Status Bar (connection status, sync indicator)     â”‚
...
Alpha: Single uvicorn process
  â””â”€â”€ Handles all API + agent work in one process

Beta: Horizontal scaling with workers
  â”œâ”€â”€ 2-4 uvicorn workers (gunicorn)
  â”œâ”€â”€ Shared-nothing (no in-memory state between workers)
  â”œâ”€â”€ Event bus becomes Redis Pub/Sub (cross-worker)
  â””â”€â”€ Rate limiting moves to Redis

Gamma: Separation of concerns
  â”œâ”€â”€ API servers (stateless, horizontally scalable)
  â”œâ”€â”€ Agent workers (separate process, dedicated GPU if needed)
  â”œâ”€â”€ Scheduler (single instance, leader election for HA)
  â””â”€â”€ Event bus â†’ RabbitMQ or Kafka
```

##### Database Layer
```
Alpha: Shared Supabase instance (single schema per domain)
  â”œâ”€â”€ Indexes on frequently queried columns (user_id, status, created_at)
  â”œâ”€â”€ Connection pooling via Supabase pooler (PgBouncer)
  â””â”€â”€ RLS for multi-user isolation

Beta: Supabase read replicas
  â”œâ”€â”€ Read replicas for dashboard/analytics queries
  â””â”€â”€ Primary instance for write-heavy domains (tasks, habits)

Gamma: Domain-specific DB optimization
  â”œâ”€â”€ Task/Knowledge: Full Supabase instance
  â”œâ”€â”€ Analytics: Separate read-optimized DB (could be ClickHouse)
  â”œâ”€â”€ AI: Vector DB (pgvector or standalone Qdrant/Pinecone)
  â””â”€â”€ Search: Dedicated search index (MeiliSearch or Elasticsearch)
```

##### Caching Strategy
```
Alpha: In-memory TTL cache (package/shared/utils/cache.py)
  â”œâ”€â”€ Cache prompt files (loaded once)
  â”œâ”€â”€ Cache user settings (5 min TTL)
  â””â”€â”€ Cache dashboard aggregates (1 min TTL)

Beta: Redis cache layer
  â”œâ”€â”€ Session cache
  â”œâ”€â”€ API response cache (10-60s TTL for read-heavy endpoints)
  â”œâ”€â”€ Rate limiter state
  â””â”€â”€ Event bus pub/sub between workers

Gamma: Multi-tier cache
  â”œâ”€â”€ L1: In-memory (hot data, <100ms TTL)
  â”œâ”€â”€ L2: Redis (warm data, <5min TTL)
  â””â”€â”€ L3: CDN (static assets, long TTL)
```
...
1. **Every new feature must be a bounded domain or extension of existing domain** â€” no god modules
2. **Every integration must go through the event bus** â€” no direct third-party API calls from domains
3. **Every expansion must support graceful degradation** â€” if the new feature can't load, existing features work
4. **Every expansion must be disable-able** â€” feature flags for all new capabilities
5. **Extensions don't break the prompt system** â€” prompts evolve independently from code
...
|---|---|---|
| Authentication | âœ… Implemented | Supabase Auth (Google OAuth, email/password) |
| Authorization (RBAC) | âš ï¸ Basic | User-level only; no role-based access yet |
| RLS Policies | âœ… Implemented | All tables have RLS with `user_id = auth.uid()` |
| Data Encryption (at rest) | âœ… Implemented | Supabase PostgreSQL encryption at rest |
| Data Encryption (in transit) | âœ… Implemented | HTTPS enforced, Supabase TLS |
| Secrets Management | âš ï¸ Manual | `.env` files; Vault planned for gamma |
| Audit Logging | âš ï¸ Partial | `analytics.event_log` stores events; no admin audit trail |
| Rate Limiting | âœ… Implemented | 100 req/min per IP in middleware |
| Input Validation | âœ… Implemented | Pydantic models, Zod schemas |
| SQL Injection Protection | âœ… Implemented | Supabase SDK parameterized queries |
| CORS | âœ… Implemented | Configurable `CORS_ORIGINS` env var |
| Session Management | âœ… Implemented | JWT-based, configurable expiry |
| API Key Rotation | âš ï¸ Manual | No automated rotation yet |
| Compliance (GDPR) | âš ï¸ Partial | Data export available; deletion policy pending |

### 13.2 Reliability

| Criterion | Status | Notes |
|---|---|---|
| Graceful Degradation (AI) | âœ… Implemented | Ollama â†’ Claude â†’ Algorithmic |
| Graceful Degradation (Offline) | âœ… Implemented | IndexedDB cache + sync queue |
| Retry Logic | âš ï¸ Partial | Event dispatch retries (3 attempts); API retries manual |
| Circuit Breaker | âŒ Missing | No circuit breaker for Ollama/Claude |
| Health Checks | âš ï¸ Basic | FastAPI `/health` endpoint; no deep checks |
| Backup & Restore | âœ… Implemented | Supabase daily backups (pro plan) |
| Disaster Recovery | âš ï¸ Documented | Runbook exists but not tested |
| SLA | âš ï¸ Defined | RPO <1s, RTO <5min for stateless components |
| Monitoring | âš ï¸ Basic | Railway + Supabase dashboards; no custom alerting |
| Alerting | âŒ Missing | No PagerDuty/Opsgenie integration |

### 13.3 Performance

| Criterion | Status | Notes |
|---|---|---|
| API Latency (p95) | âš ï¸ Not measured | Target <200ms for non-AI endpoints |
| AI Latency (p95) | âš ï¸ Not measured | Target <10s for Ollama, <3s for Claude |
| Page Load Time (FCP) | âš ï¸ Not measured | Target <2s (PWA with service worker cache) |
| Search Latency | âš ï¸ Not measured | Target <500ms for hybrid search |
| Database Query Time (p95) | âš ï¸ Not measured | Target <50ms for indexed queries |
| Concurrent Users Supported | âš ï¸ Estimated | 50-200 concurrent in gamma (Section 12.5) |
| Uptime | âš ï¸ Unofficial | Target 99.5% (no SLA yet) |

### 13.4 Maintainability

| Criterion | Status | Notes |
|---|---|---|
| Test Coverage | âš ï¸ Partial | 30+ tests (prompts + agents); API endpoints untested |
| Documentation | âœ… Extensive | ~120 docs files, ~16MB |
| Code Comments | âœ… Minimal | Per style guide: no unnecessary comments |
| Linting (Python) | âœ… Implemented | Ruff |
| Linting (TypeScript) | âœ… Implemented | ESLint |
| Type Checking (TS) | âœ… Implemented | TypeScript strict mode |
| CI/CD | âœ… Implemented | 4 CI jobs (frontend, backend, prompts, security) |
| Dependency Management | âœ… Partial | requirements.txt + package.json; no lockfile for Python |
| Prompt Versioning | âœ… Implemented | YAML frontmatter `version` field |
| Feature Flags | âŒ Missing | No feature flag system |
...
1. **Instrument API latency metrics** â€” Add timing middleware to all API endpoints; log p95 latency per endpoint. Currently no performance baselines exist.
2. **Add circuit breaker for Ollama** â€” If Ollama fails 3 times in 5 minutes, skip directly to algorithmic fallback for the next 5 minutes. Prevents cascading timeout failures.
3. **Implement feature flags** â€” Even a simple JSON file (`config/features.json`) enables gradual rollout of new capabilities.
4. **Add health check depth** â€” Extend `/health` to check Supabase connectivity, Ollama responsiveness, event bus health.

### Short-Term (Next Sprint)

5. **In-process event bus (asyncio.Queue)** â€” Replace direct function calls for cross-domain side effects with the event abstraction defined in Section 11.1. This is the first step toward event-driven decoupling without adding infrastructure.
6. **API endpoint test coverage** â€” Write integration tests for all ~53 endpoints. Currently only prompt loader and agent tests exist.
7. **Search indexing pipeline** â€” Implement the `search.documents` table and indexing subscriber. Search is currently limited to direct Supabase queries.
8. **Memory confidence decay** â€” Implement the decay algorithm (Section 11.4). Memories currently persist at full confidence indefinitely.

### Medium-Term (Next 2-3 Sprints)

9. **Redis cache + pub/sub** â€” Deploy Redis (Upstash free tier) for cross-worker event bus, rate limiter state, and API response caching.
10. **Analytics read replica** â€” Configure Supabase read replica for dashboard queries. Prevents dashboard performance from impacting write-heavy domains.
11. **Agent worker pool** â€” Extract AI agent execution into a separate worker process with a job queue. Prevents agent latency from blocking API request handling.
12. **Knowledge graph visualization** â€” Build the force-directed graph view for knowledge items with AI-suggested connections.

### Long-Term (Gamma Phase)

13. **RabbitMQ event bus** â€” Full production event bus with persistence, retry, DLQ, and replay capability.
14. **Team collaboration** â€” Shared workspaces with RBAC. This is the largest architectural change â€” requires User Domain expansion, shared data models, and collaboration-aware RLS policies.
15. **Calendar integration** â€” Two-way sync with Google Calendar/Outlook for task scheduling and deadline management.
16. **Browser extension** â€” Web clipper and task quick-add for frictionless input.
...
| Goal (D04) | â€” | Planner (A01) |
| Roadmap (D05) | â€” | Roadmap (A08) |
...
- [ ] Domain boundary respected â€” no direct cross-domain DB access
