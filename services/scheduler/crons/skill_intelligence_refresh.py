...
async def run_skill_intelligence_refresh():
    ...
    marked = supabase.from_("skill_market_data").update({"data_freshness": "refreshing"}).eq("data_freshness", "stale").execute()
    logger.info(
    ...
    if stale_skills:
        supabase.from_("skill_market_data").update({"data_freshness": "current"}).eq("data_freshness", "refreshing").execute()
